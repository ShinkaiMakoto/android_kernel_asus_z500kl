	...
	case MDSS_EVENT_UNBLANK:
#if defined(CONFIG_IRIS2_FULL_SUPPORT)
		if (boot_cnt == 0) {
		...
		if (boot_cnt == 0) {
			if (iris2_get_chip_version() >= IRIS2P_50) {
				iris_fw_download_cont_splash(ctrl_pdata, 1);
			}
			boot_cnt = 1;
		}
#endif
...
#if defined(TARGET_PROJECT_Z581KL_DISPLAY) || defined(TARGET_PROJECT_ZT581KL_DISPLAY) || defined(TARGET_PROJECT_ZT582KL_DISPLAY)
int asus_panel_id = TIANMA_R63319;
int get_asus_panel_id(void){
    return asus_panel_id;
}
#elif defined(TARGET_PROJECT_ZT500KL_DISPLAY) || defined(TARGET_PROJECT_Z500KL_DISPLAY)
int asus_panel_id = TIANMA_NT35523;
int get_asus_panel_id(void){
    return asus_panel_id;
}
...
#if defined(TARGET_PROJECT_Z581KL_DISPLAY) || defined(TARGET_PROJECT_ZT581KL_DISPLAY) || defined(TARGET_PROJECT_ZT500KL_DISPLAY) || defined(TARGET_PROJECT_Z500KL_DISPLAY) || defined(TARGET_PROJECT_ZT582KL_DISPLAY)
		...
		struct platform_device *pdev)
{
...
#if defined(TARGET_PROJECT_Z581KL_DISPLAY) || defined(TARGET_PROJECT_ZT581KL_DISPLAY) || defined(TARGET_PROJECT_ZT582KL_DISPLAY)
	int panel_id_gpio;
	int panel_id_gpio_value=-1;
	int rc;

	panel_id_gpio = of_get_named_gpio(pdev->dev.of_node, "qcom,panel-id-gpio", 0);
	if (!gpio_is_valid(panel_id_gpio))
		pr_info("%s: panel_id_gpio gpio not specified\n", __func__);
	else {
		rc = gpio_request(panel_id_gpio, "panel_id_gpio");
		if (rc) {
			pr_err("request panel_id_gpio gpio failed, rc=%d\n", rc);
		}
		panel_id_gpio_value = gpio_get_value(panel_id_gpio);
	}

	if(panel_id_gpio_value==0){
		asus_panel_id = SHARP_NT35523;
	}else{
		asus_panel_id = TIANMA_R63319;
	}

	printk("DISPLAY: panel_id=%d, asus_panel_id=%d\n", panel_id_gpio_value, asus_panel_id);
#elif defined(TARGET_PROJECT_ZT500KL_DISPLAY)
	...
	if(panel_id_gpio_value==0){ //BOE_NT35523
		asus_panel_id = BOE_NT35523;
	}else{ // TIANMA
		if(hw_id==0){
			asus_panel_id = TIANMA_NT35523;
		}else{
			asus_panel_id = TIANMA_NT35523_ES13;
		}

	}
	printk("DISPLAY: panel_id_gpio_value=%d\n", panel_id_gpio_value);
	printk("DISPLAY: hw_id=%d, asus_panel_id=%d\n", hw_id, asus_panel_id);
#elif defined(TARGET_PROJECT_Z500KL_DISPLAY)
	int hw_id = Read_HW_ID();
	int panel_id_gpio;
	int panel_id_gpio_value=-1;
	int rc;
	...
	pr_debug("%s:%d: Select primary panel from dt\n",
					__func__, __LINE__);
#if defined(CONFIG_IRIS2_LIGHTUP_ONLY) || defined(CONFIG_IRIS2_FULL_SUPPORT) || defined(CONFIG_IRIS2P_FULL_SUPPORT) || defined(CONFIG_IRIS2P_LIGHTUP_ONLY)
#if defined(TARGET_PROJECT_Z581KL_DISPLAY) || defined(TARGET_PROJECT_ZT581KL_DISPLAY) || defined(TARGET_PROJECT_ZT582KL_DISPLAY)
	if(get_asus_panel_id()==SHARP_NT35523){
		dsi_pan_node = of_parse_phandle(pdev->dev.of_node,
						"qcom,dsi-pref-prim-pan-with-iris-lcm2", 0);
	}else if(get_asus_panel_id()==TIANMA_R63319){
		dsi_pan_node = of_parse_phandle(pdev->dev.of_node,
						"qcom,dsi-pref-prim-pan-with-iris", 0);
	}else{
		printk("Error: asus_panel_id = %d\n", asus_panel_id);
	}
#elif defined(TARGET_PROJECT_ZT500KL_DISPLAY)
	if(get_asus_panel_id()==TIANMA_NT35523_ES13){
		dsi_pan_node = of_parse_phandle(pdev->dev.of_node,
						"qcom,dsi-pref-prim-pan-with-iris-lcm2", 0);
	}else if(get_asus_panel_id()==TIANMA_NT35523){
		dsi_pan_node = of_parse_phandle(pdev->dev.of_node,
						"qcom,dsi-pref-prim-pan-with-iris", 0);
	}else if(get_asus_panel_id()==BOE_NT35523){
		dsi_pan_node = of_parse_phandle(pdev->dev.of_node,
						"qcom,dsi-pref-prim-pan-with-iris-lcm3", 0);
	}else{
		printk("Error: asus_panel_id = %d\n", asus_panel_id);
	}
#elif defined(TARGET_PROJECT_Z500KL_DISPLAY)
...
#endif // CONFIG_IRIS2_LIGHTUP_ONLY || CONFIG_IRIS2_FULL_SUPPORT || CONFIG_IRIS2P_FULL_SUPPORT || CONFIG_IRIS2P_LIGHTUP_ONLY
	if (!dsi_pan_node)
