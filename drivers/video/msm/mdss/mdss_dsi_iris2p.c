...
void iris_update_configure(void)
{
	...
	/*lp memc*/
	pqlt_cur_setting->lp_memc_setting.value = iris_lp_memc_calc(pqlt_cur_setting->lp_memc_setting.level);
	...
	if (pqlt_def_setting->lce_setting.mode != pqlt_cur_setting->lce_setting.mode) {
		...
		iris_info.update.lce_setting = true;
	} else if (pqlt_def_setting->lce_setting.graphics_detection != pqlt_cur_setting->lce_setting.graphics_detection) {
		pr_debug("lce graphics detection update\n");
		iris_info.update.lce_setting = true;
	}
...
bool iris_is_scale_enable(void)
{
...
static void iris_meta_wq_handler(void)
{
	...
	if (iris_meta_pkts[0].dchdr.dlen > META_HEADER)
		mdss_dsi_cmds_tx(ctrl, iris_meta_pkts, cnt, 0);
	memset(imeta, 0, sizeof(imeta));
	pr_debug("%s ------\n", __func__);
}
