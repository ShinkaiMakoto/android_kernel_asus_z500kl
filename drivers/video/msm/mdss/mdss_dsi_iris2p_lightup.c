			...
			struct dsi_panel_cmds *pcmds)
{
	...
	if (cmd_send_flag) {
		...
		mdss_dsi_cmdlist_tx(ctrl, &cmdreq);
		if (cmdreq.flags & CMD_REQ_HS_MODE)
...
#define RELOAD_FIRMWARE 2
	...
	if (SUCCESS == ret) {
		...
		for (i = 0; i < FW_DOWNLOAD_RETRYCNT_MAX; i++) {
			...
			if (result == SUCCESS)
				break;
			pr_info("firmware download again\n");
		}
		iris_firmware_download_restore(ctrl, cont_splash);
	}

	pw_iris2_status = (result == SUCCESS) ? IRIS_MODE_RFB : -1;
	return result;
}
...
#ifdef EFUSE_REWRITE
void iris_sys_efuse_rewrite(struct mdss_dsi_ctrl_pdata *ctrl)
{
	u32 grcp_len = 0;
...
void iris_init(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	iris_mipirx_mode_set(ctrl, BYPASS_CMD, DSI_LP_MODE);
...
int iris_one_wired_cmd_init(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	iris2_io_set_reset(1, 1);
#elif defined(ONE_WIRED_CMD_VIA_WAKEUP_GPIO)
	iris2_io_set_reset(1, 0);
#endif
	msleep(2);
...
#if defined(ONE_WIRED_CMD_VIA_RESET_GPIO)
		iris2_io_set_reset(0, 0);
		udelay(pulse_delay);
		iris2_io_set_reset(0, 1);
		udelay(pulse_delay);
#elif defined(ONE_WIRED_CMD_VIA_WAKEUP_GPIO)
		iris2_io_set_reset(0, 1);
		udelay(pulse_delay);
...
/* low power state machine process */
int iris_low_power_process(struct msm_fb_data_type *mfd)
{
	struct iris_config *iris_cfg = &mfd->iris_conf;
	u8 signal_mode = 0xff, mode_switch, pq_update, lp_switch;
	int *pstate = &iris_info.power_status.low_power_state;

	if (!gp_mfd->iris_conf.ready || iris_debug_power_opt_disable)
		return true;

	if (LP_STATE_POWER_UP == *pstate)
		return true;

	if (IRIS_BYPASS_MODE == iris_cfg->current_mode)
		return true;

	lp_switch = (LP_STATE_POWER_UP_PREPARE == *pstate)
		|| (LP_STATE_POWER_DOWN_PREPARE == *pstate);
	...
	pq_update = iris_info.update.pq_setting || iris_info.update.dbc_setting
		|| iris_info.update.color_adjust || iris_info.update.lce_setting
		|| iris_info.update.cm_setting || iris_info.update.cmd_setting
		|| iris_info.update.reading_mode;

	if (lp_switch || mode_switch || pq_update) {

		signal_mode = iris_mipi_signal_mode_read(g_ctrl, DSI_LP_MODE);
		if (signal_mode != 0)
			return false;

		if (LP_STATE_POWER_UP_PREPARE == *pstate) {
			*pstate = LP_STATE_POWER_UP;
			return true;
		} else if (LP_STATE_POWER_DOWN_PREPARE == *pstate) {
			*pstate = LP_STATE_POWER_DOWN;
		}

		if ((mode_switch || pq_update) && (LP_STATE_POWER_DOWN == *pstate)) {
			iris_reg_add(IRIS_MIPI_RX_ADDR + DCS_CMD_PARA_2, LP_CMD_LP_EXIT);
			*pstate = LP_STATE_POWER_UP_PREPARE;
		}
...
void iris_low_power_mode_notify(struct msm_fb_data_type *mfd)
{
	...
	iris_reg_add(IRIS_MIPI_RX_ADDR + DCS_CMD_PARA_2, LP_CMD_LP_ENTER);
	*pstate = LP_STATE_POWER_DOWN_PREPARE;
...
void iris_low_power_stop_mcu(struct mdss_dsi_ctrl_pdata *ctrl, bool stop, int state)
{
	...
	*(u16 *)(grcp_cmd.cmd + 14) = cpu_to_le16(grcp_len);
	...
	pr_info("iris low power mcu stop: %d\n", stop);
...
void iris_sys_ext_te_sel(struct mdss_dsi_ctrl_pdata *ctrl)
{
	u32 grcp_len = 0;
	struct dsi_cmd_desc iris_grcp_cmd[] = {
		{{ DTYPE_GEN_LWRITE, 1, 0, 0, 0, CMD_PKT_SIZE}, grcp_cmd.cmd},
