...
static u32 iris_ip_reg_addr = IRIS_PWIL_ADDR;
...
static int iris_configure(struct msm_fb_data_type *mfd, u32 type, u32 value)
{
	...
	case IRIS_LCE_SETTING:
		...
		pqlt_cur_setting->lce_setting.graphics_detection = (value & 0x10000) >> 16;
...
static int iris_configure_get(struct msm_fb_data_type *mfd, u32 type, u32 count, u32 *values)
{
	...
	case IRIS_DBC_CONFIG:
		...
		*values = (*values) & 0xfffffffe; // Filter update bit.
		break;
	case IRIS_CINEMA_MODE:
	...
	case IRIS_LCE_SETTING:
		...
		*values = (*values) & 0x7fffffff; // Filter update bit.
		break;
	case IRIS_CM_SETTING:
	...
	case IRIS_CHIP_VERSION:
		if (iris2_get_chip_version() < IRIS2P_50)
			*values = IRIS2_CHIP_HW_VER;
		else
			*values = IRIS2P_CHIP_HW_VER;
		break;
	case IRIS_DBG_TARGET_REGADDR_VALUE_GET:
		*values = iris_pi_read(ctrl, iris_ip_reg_addr);
		break;
	case IRIS_LUX_VALUE:
