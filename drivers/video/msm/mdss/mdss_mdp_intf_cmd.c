...
int mdss_mdp_cmd_kickoff(struct mdss_mdp_ctl *ctl, void *arg)
{
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = container_of(ctl->panel_data, struct mdss_dsi_ctrl_pdata,
                               panel_data);
#endif
...
#elif defined(CONFIG_IRIS2P_FULL_SUPPORT)
	if (iris2_get_chip_version() >= IRIS2P_50 && ctl->mfd->panel_info->is_prim_panel) {
		iris_mipitx_interface_switch(ctl->mfd, 1);
		iris_abypass_switch_proc(ctrl_pdata);
		iris_copy_meta(ctl->mfd);
		if (iris_low_power_process(ctl->mfd)) {
			iris_mode_switch_cmd(ctl->mfd);
			iris_frc_repeat(ctl->mfd);
			iris_calc_nrv(ctl);
			iris_calc_meta(ctl->mfd);
