...
// Chase Support for /proc/printk_ponreason and /proc/printk_poffreason
#include <linux/proc_fs.h>
extern void create_printk_proc(void);
#define PMIC_PROC_PONREASON_FILE "printk_ponreason"
#define PMIC_PROC_POFFREASON_FILE "printk_poffreason"
static struct proc_dir_entry *pmic_proc_file;
static int poffreason_index = 8;
...
static int qpnp_pon_store_and_clear_warm_reset(struct qpnp_pon *pon)
{
	...
	/*Chase add for debug*/
	//dev_info(&pon->spmi->dev, "PMIC@SID%d: Flysky qpnp_pon_store_and_clear_warm_reset, warm_reset_reason1 = %d, warm_reset_reason2 = %d \n", pon->spmi->sid, pon->warm_reset_reason1, pon->warm_reset_reason2);
...
struct delayed_work mw_work;
int KPDPWR_ST=0;
#endif

#if !defined(VZW)
static void reset_PON_work(struct work_struct *work){
        pr_err("[%s] KPDPWR_ST:%d...\n",__func__,KPDPWR_ST);
        if (KPDPWR_ST){
                kernel_restart("Press KPDPWR to trigger soft reset");
        } else{
                cancel_delayed_work(&mw_work);
        }
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	if (!cfg->old_state && !key_status) {
        ...
        if (cfg->pon_type == PON_KPDPWR){
            if (key_status){
                printk(KERN_ERR "[Buttton]Power key press - value:%d\n",key_status);
                wake_lock(&pon->Event_wakelock);
#if !defined(VZW)
                /* Do not set duplicate timer if timer is pending */
                if (!timer_pending(&mw_work.timer)) {
		    KPDPWR_ST = 1;
		    queue_delayed_work(mw_wq,&mw_work,6*HZ);
		}
#endif
            } else {
#if !defined(VZW)
		/* Avoid to cancel first delayed work even if power key is pressed */
		if (KPDPWR_ST == 1) {
		    KPDPWR_ST = 0;
		    cancel_delayed_work(&mw_work);
		}
#endif
		wake_unlock(&pon->Event_wakelock);
		printk(KERN_ERR "[Buttton]Power key release - value:%d\n",key_status);
            }
        }
...
#ifndef USER_BUILD
        if((cfg->pon_type == PON_RESIN) && !key_status)
		...
		case PON_KPDPWR:
			...
			if (rc) {
				...
				cfg->config_reset = true;
                        ...
                        /* Due to VZW request, disable the reset behavior on PON_KPDPWR */
                        #if defined(CONFIG_COMBINATION)
		...
		case PON_KPDPWR_RESIN:
			...
			if (rc) {
				...
				cfg->config_reset = true;
                        ...
                        /* Due to VZW request, disable the reset behavior on PON_KPDPWR_RESIN */
                        #if !defined(CONFIG_COMBINATION)
                            cfg->support_reset = false;
                        #endif
		...
		if (cfg->support_reset) {
			...
			if (rc) {
				...
				return rc;
                        ...
                        #if !defined(VZW)
			    cfg->s2_type = 0x5; /* Set S2 type to DvDD shutdown */ 
                        #endif
...
static int qpnp_pon_probe(struct spmi_device *spmi)
{
	...
	/*Chase add for debug*/
	//dev_info(&pon->spmi->dev, "PMIC@SID%d: Flysky power-on, pon_sts = %d, index = %d \n", pon->spmi->sid, pon_sts, index);
	...
	/*Chase add for debug*/
	//dev_info(&pon->spmi->dev, "PMIC@SID%d: Flysky power-off, poff_sts = %d, index = %d \n", pon->spmi->sid, poff_sts, index);
	...
	// Chase add to createpower-on reason and  power-off reason
        create_printk_proc();
	qpnp_pon_debugfs_init(spmi);
        wake_lock_init(&pon->Event_wakelock, WAKE_LOCK_SUSPEND, "power_button");

#if !defined(VZW)
        /* Create delay workqueue for detecting long press power button */
        mw_wq = create_singlethread_workqueue("MW_workqueue");
        INIT_DEFERRABLE_WORK(&mw_work, reset_PON_work);
        queue_delayed_work(mw_wq,&mw_work,60*HZ);
#endif

	return 0;
}

// Chase add printk_ponreason and printk_poffreason ++
static int pmic_proc_ponreason_read(struct seq_file *buf, void *v)
{
        seq_printf(buf, "%s\n", cold_boot ? "cold" : "warm");
        return 0;
}

static int pmic_proc_ponreason_open(struct inode *inode, struct  file *file) {
        return single_open(file, pmic_proc_ponreason_read, NULL);
}

static int pmic_proc_poffreason_read(struct seq_file *buf1, void *v)
{
        seq_printf(buf1, "%s\n", qpnp_poff_reason[poffreason_index]);
        return 0;
}

static int pmic_proc_poffreason_open(struct inode *inode, struct  file *file) {
        return single_open(file, pmic_proc_poffreason_read, NULL);
}

static const struct file_operations pmic_ponreason_fops = {
        .owner = THIS_MODULE,
        .open = pmic_proc_ponreason_open,
        .read = seq_read,
};

static const struct file_operations pmic_poffreason_fops = {
        .owner = THIS_MODULE,
        .open = pmic_proc_poffreason_open,
        .read = seq_read,
};

void create_printk_proc(void) {
        pmic_proc_file = proc_create(PMIC_PROC_PONREASON_FILE, 0666, NULL, &pmic_ponreason_fops);
        if(pmic_proc_file){
                printk("PMIC proc ponreason file create sucessed!\n");
        }
        else{
                printk("PMIC proc ponreason file create failed!\n");
        }
        pmic_proc_file = proc_create(PMIC_PROC_POFFREASON_FILE, 0666, NULL, &pmic_poffreason_fops);
        if(pmic_proc_file){
                printk("PMIC proc poffreason file create sucessed!\n");
        } else {
                printk("PMIC proc poffreason file create failed!\n");
        }
}
