						...
						struct sdhci_host *host)
{
	...
	if (!of_property_read_u8(np, "cd-gpios-pins", &num_cd_pins)) {
		hardware_phase = Read_HW_ID_STR();
		dev_err(dev, "hardware phase: %s\n", hardware_phase);
		gpio_idx = of_property_match_string(np, "cd-gpios-hw_phase", hardware_phase);
		if( gpio_idx < 0)
			gpio_idx = 0;
	}
	pdata->status_gpio = of_get_named_gpio_flags(np, "cd-gpios", gpio_idx, &flags);
	...
	if (of_property_read_bool(np, "qcom,wakeup-on-idle"))
        ...
        if (of_find_property(np, "cap-hw-reset", NULL))
			...
			bool enable, bool is_init)
{
	...
	if (!is_init && enable && gpio_is_valid(pdata->status_gpio)) {
		i = !!gpio_get_value(pdata->status_gpio);
		i ^= !!(pdata->caps2 & MMC_CAP2_CD_ACTIVE_HIGH);
		if (i)	{
			/* Don't enable VDD regulator for SD since this is combo socket, if
			 * card is absent. Avoid damage nano SIM. */
			vreg_table[0] = NULL;
			pr_info("%s: bypass enabling VDD regulator due to card absent\n", __func__);
		}
	}
...
static void sdhci_msm_check_power_status(struct sdhci_host *host, u32 req_type)
{
	...
	if (req_type == REQ_BUS_ON && !mmc_gpio_get_cd(msm_host->mmc)) {
		/* Enableing for VDD regulator is bypassed, if card absent. */
		done = true;
	}
	/*
...
static void mmc1_sdhci_hw_reset(struct sdhci_host *host)
{
	int rc;
#if defined (CONFIG_MMC_SDHCI_MSM)
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_msm_host *msm_host = pltfm_host->priv;
#endif
	...
	if (rc) {
		pr_err("%s: %s unable to disable regulators. errno=%d\n",
		       mmc_hostname(host->mmc), __func__, rc);
	}
	/* 10ms delay */
	usleep_range(10000, 12000);
	...
	if (rc) {
		pr_err("%s: %s unable to enable regulators. errno=%d\n",
		       mmc_hostname(host->mmc), __func__, rc);
	}
#endif
	/* 10ms delay */
...
static ssize_t SD_status(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	status ^= !(host->mmc->caps2 & MMC_CAP2_CD_ACTIVE_HIGH);
	return snprintf(buf, PAGE_SIZE, "%d\n", status);
}
...
#define PRE_EOL_INFO_SLICE 267
static ssize_t eMMC_health(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
	struct mmc_host *mmc = host->mmc;
	...
	mmc_rpm_hold(card->host, &card->host->class_dev);
	...
	if (mmc && mmc->card && mmc_card_mmc(card)){
		err = mmc_send_ext_csd(card, ext_csd);
		if (err)
			goto out_unhalt;
	}
	written = scnprintf(buf, PAGE_SIZE, "0x%02x\n", (u8)ext_csd[PRE_EOL_INFO_SLICE]);
...
out:
	...
	mmc_rpm_release(card->host, &card->host->class_dev);
	kfree(ext_csd);
	return written;
}
...
static DEVICE_ATTR(emmc_health, S_IRUGO, eMMC_health, NULL);
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	pr_info("%s: caps=0x%08x, caps2=0x%08x, pm_caps=0x%08x, quirks=0x%08x, quirks2=0x%08x\n",
		mmc_hostname(host->mmc), host->mmc->caps,
		host->mmc->caps2, host->mmc->pm_caps, host->quirks, host->quirks2);
