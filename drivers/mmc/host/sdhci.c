                                      ...
                                      struct mmc_card *card)
{
	...
	bool disable_cq = false;
		...
		if (prj_id == PROJ_ID_ZT581KL) {
			mid = (card->raw_cid[0] >> 24) & 0xff;
			prv = (card->raw_cid[2] >> 16) & 0xff;
			if (mid == CID_MANFID_HYNIX) {
				if (hw_id == HW_ID_ER || hw_id == HW_ID_EVB || hw_id == HW_ID_SR) {
					if (prv == 0xa4 && card->ext_csd.fw_version == 0xa4) {
						/* Disable CQ for ZT581KL/ER/Hynix/PRV=0xa4/fw_version=0xa4.
						 * Since Hynix made things wrong. We will have two firmwares,
						 * both of them are 0xa4, only particles mounted at post ER devices
						 * can support CQ.
						 *
						 * BTW, we also have a test firmware for internal test which
						 * PRV is 0xa4, but fw_version is 0xf5. CQ should be enabled
						 * for it.
						 * */
						disable_cq = true;
					}
				}
			} else if (CID_MANFID_SAMSUNG) {
				if (hw_id == HW_ID_PR || hw_id == HW_ID_ER || hw_id == HW_ID_EVB || hw_id == HW_ID_SR) {
					if (prv == 0x07 && card->ext_csd.fw_version == 0x07) {
						/* Disable CQ for ZT581KL non-MP devices which is using Samsung particle,
						 * Both of PRV and fw_version are 0x07 */
						disable_cq = true;
					}
				}
			}
		...
		if (disable_cq) {
			...
			pr_info("%s: disable CQ due to buggy eMMC firmware.\n", mmc_hostname(card->host));
		}
        }
}
