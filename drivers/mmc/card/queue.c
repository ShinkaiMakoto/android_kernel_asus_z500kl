...
int mmc_cmdq_init(struct mmc_queue *mq, struct mmc_card *card)
{
	...
	/* rq_timeout is initialied as 8 seconds. It will be revert to 120 after
	 * succeed in adding disk in mmc_blk_probe.
	 * This is WA to deal with eMMC timed out error.
	 * If partition is not load yet, it will triger timeout error quickly.
	 * Thus the error handling take in place. */
	blk_queue_rq_timeout(mq->queue, 8 * HZ);
