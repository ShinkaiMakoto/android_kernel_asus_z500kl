...
static void gic_show_resume_irq(struct gic_chip_data *gic)
{
	...
	void __iomem *base = gic_data_dist_base(gic);
        ...
        unsigned int gic_buf[MAX_GIC_REASON];
        unsigned int j=0,k=0;
        unsigned int flag=0;

        memset(gic_buf,0,sizeof(unsigned int)*MAX_GIC_REASON);
	...
	i = find_next_bit((unsigned long *)pending, gic->gic_irqs, i+1)) {
		...
		pr_warning("%s: %d triggered %s\n", __func__,
                ...
                if (k < MAX_GIC_REASON)
                    gic_buf[k++] = i + gic->irq_offset;
	}
        for (j=0; j<k; j++) {
            switch(gic_buf[j]){
                case 57:   /* trigger from modem */
                case 178:  /* trigger from WiFi */
                case 222:  /* trigger from PMIC (EX: volume down, power key...) */
                case 240:  /* trigger from GPIO (EX: volume up, touch, USB...) */
                    log_wakeup_reason(gic_buf[j]);
                    flag=1;
            }
            if (flag) break;
        }
        /*
        Logging all the wakeup reason if it doesn't contain the above IRQ number.
        EX: RTC=>qcom,smd-rpm & 601d0.qcom,mpm  , watchdog pet=> qcom,smd-rpm , ADSP=> qcom,smd-rpm & qcom,smd-adsp
        */
        if (!flag){
            for (j=0; j<k; j++) log_wakeup_reason(gic_buf[j]);
        }
}
