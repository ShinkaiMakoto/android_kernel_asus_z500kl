...
static int choose_times(int msec, int *c1p, int *c2p)
{
	...
	int diff = 65536;
	if (DEBUG_LOG)
		printk(KERN_EMERG "[TCA6507][%s] msec(%d)\n", __func__, msec);
...
static void set_select(struct tca6507_chip *tca, int led, int val)
{
	...
	int bit;
	if (DEBUG_LOG)
		printk(KERN_EMERG "[TCA6507][%s] led(%d) val(%d)\n", __func__, led, val);
	for (bit = 0; bit < 3; bit++) {
...
static void set_code(struct tca6507_chip *tca, int reg, int bank, int new)
{
	...
	int n;
	if (DEBUG_LOG)
		printk(KERN_EMERG "[TCA6507][%s] bank(%d) new(%d)\n", __func__, bank, new);
	if (bank) {
		mask <<= 4;
		new <<= 4;
	}
...
{
	if (DEBUG_LOG)
		printk(KERN_EMERG "[TCA6507][%s] bank(%d) level(%d)\n", __func__, bank, level);
	switch (bank) {
	case BANK0:
	case BANK1:
...
static void set_times(struct tca6507_chip *tca, int bank)
{
	...
	int result;
	if (DEBUG_LOG)
		printk(KERN_EMERG "[TCA6507][%s] bank(%d)\n", __func__, bank);
	result = choose_times(tca->bank[bank].ontime, &c1, &c2);
	dev_dbg(&tca->client->dev,
		"Chose on  times %d(%d) %d(%d) for %dms\n", c1, time_codes[c1],
		c2, time_codes[c2], tca->bank[bank].ontime);
	if (DEBUG_LOG)
		printk(KERN_EMERG "[TCA6507]Chose on  times %d(%d) %d(%d) for %dms\n", c1, time_codes[c1], c2, time_codes[c2], tca->bank[bank].ontime);
//	set_code(tca, TCA6507_FADE_ON, bank, c2);
//	set_code(tca, TCA6507_FULL_ON, bank, c1);
	set_code(tca, TCA6507_FADE_ON, bank, 0);
	set_code(tca, TCA6507_FULL_ON, bank, c2+1);
	...
	if (DEBUG_LOG)
		printk(KERN_EMERG "[TCA6507] Chose off times %d(%d) %d(%d) for %dms\n", c1, time_codes[c1], c2, time_codes[c2], tca->bank[bank].offtime);
//	set_code(tca, TCA6507_FADE_OFF, bank, c2);
//	set_code(tca, TCA6507_FIRST_OFF, bank, c1);
//	set_code(tca, TCA6507_SECOND_OFF, bank, c2);
	set_code(tca, TCA6507_FADE_OFF, bank, 0);
	set_code(tca, TCA6507_FIRST_OFF, bank, c2);
	set_code(tca, TCA6507_SECOND_OFF, bank, c2);
				   ...
				   enum led_brightness brightness)
{
	struct tca6507_led *led = container_of(led_cdev, struct tca6507_led,
					       led_cdev);
	if (DEBUG_LOG)
		printk(KERN_EMERG "[TCA6507][%s] brightness(%d)\n", __func__, brightness);
			     ...
			     unsigned long *delay_off)
{
	struct tca6507_led *led = container_of(led_cdev, struct tca6507_led,
					       led_cdev);
	if (DEBUG_LOG)
		printk(KERN_EMERG "[TCA6507][%s] delay_on(%ld) delay_off(%ld)\n", __func__, *delay_on, *delay_off);
	if (*delay_on == 0)
		led->on_dflt = 1;
...
u32 g_delay_on=0, g_delay_off=0;
...
{
        ssize_t ret;
        ret = kstrtou32(buf, 10, &g_delay_on);
        if (ret)
                return ret;
	if (DEBUG_LOG)
		printk(KERN_ERR "[TCA6507][%s]\n", __func__);
        ...
        const char *buf, size_t count)
{
	...
	if (DEBUG_LOG)
		printk(KERN_ERR "[TCA6507][%s]\n", __func__);
        ret = kstrtou32(buf, 10, &g_delay_off);
        if (ret)
                return ret;
        return count;
        ...
        const char *buf, size_t count)
{
	...
	struct tca6507_led *led = container_of(led_cdev, struct tca6507_led, led_cdev);
	unsigned long *on , *off, tmp_on, tmp_off;

	if (DEBUG_LOG)
		printk(KERN_ERR "[TCA6507][%s] %s\n", __func__, buf);
	tmp_on = (unsigned long)g_delay_on;
	on = &tmp_on;
	tmp_off = (unsigned long)g_delay_off;
	off = &tmp_off;

	tca6507_blink_set(&led->led_cdev, on, off);

        return count;
}

static DEVICE_ATTR(pwm_us, 0666, NULL, pwm_us_store);
static DEVICE_ATTR(delay_on, 0666, NULL, delay_on_store);
static DEVICE_ATTR(delay_off, 0666, NULL, delay_off_store);
		...
		const struct i2c_device_id *id)
{
	...
	for (i = 0; i < NUM_LEDS; i++) {
		...
		if (pdata->leds.leds[i].name && !pdata->leds.leds[i].flags) {
			...
			if (err < 0)
				goto exit;
			err = sysfs_create_group(&l->led_cdev.dev->kobj, &pwm_attr_group);
			if (err < 0)
				goto exit;
		}
	}
...
static int tca6507_remove(struct i2c_client *client)
{
	...
	for (i = 0; i < NUM_LEDS; i++) {
		if (tca_leds[i].led_cdev.name){
			...
			sysfs_remove_group(&tca_leds[i].led_cdev.dev->kobj, &pwm_attr_group);
		}
	}
