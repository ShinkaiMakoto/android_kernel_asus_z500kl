...
int smb1351_read_register(int reg, u8 val)
{
	//struct i2c_client *client = NULL;
	//struct smb1351_charger *chip = container_of(client, struct smb1351_charger, client);
	int rc = 0;
	
	rc = smb1351_read_reg(smb1351_chip, reg, &val);
	if (rc) {
...
EXPORT_SYMBOL(smb1351_read_register);
...
int smb1351_write_register(int reg, u8 val)
{
	//struct i2c_client *client = NULL;
	//struct smb1351_charger *chip = container_of(client, struct smb1351_charger, client);
	int rc = 0;
	
	rc = smb1351_write_reg(smb1351_chip, reg, val);
	if (rc) {
...
EXPORT_SYMBOL(smb1351_write_register);
...
int smb1351_masked_write_register(int reg, u8 mask, u8 val)
{
	int rc = 0;
	
	rc = smb1351_masked_write(smb1351_chip, reg, mask, val);
	if (rc) {
		pr_err("masked write failed: reg=%03X, rc=%d\n", reg, rc);
		return rc;
	}

	return 0;
}
EXPORT_SYMBOL(smb1351_masked_write_register);
...
static int smb1351_get_prop_batt_status(struct smb1351_charger *chip)
{
	...
	if (chip->batt_full){
		pr_err("POWER_SUPPLY_STATUS_FULL\n");
...
static int show_smb1351_regs(struct seq_file *m, void *data)
{
	struct smb1351_charger *chip = m->private;
	int rc;
	u8 reg;
	...
	seq_printf(m, "== command_registers ==\n");
	for (addr = FIRST_CMD_REG; addr <= LAST_CMD_REG; addr++) {
		rc = smb1351_read_reg(chip, addr, &reg);
		if (!rc)
			seq_printf(m, "0x%02x = 0x%02x\n", addr, reg);
...
static int smb1351_debugfs_open(struct inode *inode, struct file *file)
{
	struct smb1351_charger *chip = inode->i_private;

	return single_open(file, show_smb1351_regs, chip);
}

static const struct file_operations smb1351_debugfs_ops = {
	.owner		= THIS_MODULE,
	.open		= smb1351_debugfs_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
static int create_debugfs_entries(struct smb1351_charger *chip)
{
	...
	if (!chip->debug_root) {
		...
		ent = debugfs_create_file("smb1351_regs", S_IFREG | S_IRUGO,
					  chip->debug_root, chip,
					  &smb1351_debugfs_ops);
		if (!ent)
			pr_err("Couldn't create cmd debug file\n");
