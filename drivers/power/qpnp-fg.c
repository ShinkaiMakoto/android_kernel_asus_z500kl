...
/* Add by Shilun for UI battery version */
#define BATTERY_VERSION "C11P1608-G-01-0002-14.9.0.14"
...
static struct fg_mem_setting settings[FG_MEM_SETTING_MAX] = {
...
#else
	SETTING(SOFT_COLD,       0x454,   0,      15),
	SETTING(SOFT_HOT,        0x454,   1,      600),
#endif
	...
	SETTING(CHG_TERM_CURRENT, 0x4F8,   2,      200),
...
struct fg_chip {
	...
	/* Add by Shilun for UI battery version */
	struct switch_dev battery_sdev;
#if defined(ENG_BUILD)
	struct delayed_work read_charging_limit_work;
#endif
};
...
/* AC charging Alarm, shilun_huang@asus.com */
extern atomic_t AC_charging_alarm;
struct wake_lock AC_charging_lock;
extern bool b_eng_charging_limit;
extern int jeita_control(int i_bat_capacity, int i_bat_temp, int i_bat_volt);
/* remapping capacity by asus, shilun_huang@asus.com*/
...
void mapping_init(void){
	...
	if (charge_temp != 256 || discharge_temp != 256){
		printk(KERN_ERR "mapping error, # of charge_temp = %d, # of discharge_temp = %d \n", 
	...
	/* print mapping table */
/*
	printk(KERN_ERR "########## ASUS charge mapping table ###########\n"); 
	for (i=255; i>=0; i--){
		printk(KERN_ERR "ADC[%d] %d %%\n", i, mapping_charge[i]);
	}
	printk(KERN_ERR "#########################################\n"); 
	printk(KERN_ERR "########## ASUS discharge mapping table ###########\n"); 
	for (i=255; i>=0; i--){
	...
	if (input_present){
		if (pre_usb_present == input_present){
			...
			if( mapping_charge[adc_temp] >= pre_capacity ){
				...
				return mapping_charge[adc_temp];
			}
			else
				return mapping_discharge[adc_temp];
		}
	}
	else{
		if (pre_usb_present == input_present){
...
#if defined(VZW)
int g_soc_in, g_soc_out, g_soc_diff;
EXPORT_SYMBOL(g_soc_in);
EXPORT_SYMBOL(g_soc_out);
EXPORT_SYMBOL(g_soc_diff);
int get_capacity_remapping_in(void)
{
	...
	g_soc_out = 0;
	g_soc_diff = 0;
	g_soc_in = get_prop_capacity_asus(chip);
	pr_err("g_soc_in = %d\n", g_soc_in);
	return g_soc_in;
}
int get_capacity_remapping_out(void)
{
	struct power_supply *bms_psy = power_supply_get_by_name("bms");
	struct fg_chip *chip = container_of(bms_psy, struct fg_chip, bms_psy);
	g_soc_out = get_prop_capacity_asus(chip);
	g_soc_diff = g_soc_out - g_soc_in;
	pr_err("g_soc_out = %d, g_soc_diff = %d\n", g_soc_out, g_soc_diff);
	
	return g_soc_out;
}
EXPORT_SYMBOL(get_capacity_remapping_in);
EXPORT_SYMBOL(get_capacity_remapping_out);
#endif
#define HIGH_BIAS	3
...
// Declare the update_sram_data() function before using it
...
static int get_sram_prop_now(struct fg_chip *chip, unsigned int type)
{
	...
	if (type == FG_DATA_CURRENT)
	{
		...
		msleep(100);
		//pr_info("curren_now %d ###\n", -(fg_data[FG_DATA_CURRENT].value/1000));
	}
...
static int get_demo_mode_status(void)
{
	struct file* filp = NULL;
	char read_buf[1];
	
	filp = filp_open(DEMO_MODE_STATUS_FILE_PTAH, O_RDONLY, 0);
	if(IS_ERR_OR_NULL(filp))
	{
		//pr_info("%s file open fail ###\n", DEMO_MODE_STATUS_FILE_PTAH);
		return DEMO_MODE_DISABLE;
	}
	
	filp->f_op->read(filp, read_buf, sizeof(read_buf), &filp->f_pos);
	//pr_info("demo mode status is %s ###\n", read_buf);
	
	if(!strncmp(read_buf, "1", 1))
	{
		//pr_info("read demo mode status result is enable ###\n");
		filp_close(filp, NULL);
		return DEMO_MODE_ENABLE;
	}
	else
	{
		//pr_info("read demo mode status result is not enable ###\n");
		filp_close(filp, NULL);
		return DEMO_MODE_DISABLE;
	}
...
*/
#if defined(ENG_BUILD)
extern int charging_limit_threshold;
extern int dual_chg_disable(void);
static void read_charging_limit_from_file(struct work_struct *work)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	char backup_fbuf[4];
	...
	if (!IS_ERR_OR_NULL(fp)){
		old_fs = get_fs();
		set_fs(KERNEL_DS);
		fp->f_op->read(fp, backup_fbuf, sizeof(backup_fbuf), &fp->f_pos);
		sscanf(backup_fbuf,"%d",&charging_limit_threshold);
		pr_err("read data from file: %d\n", charging_limit_threshold);
	}
	else
		pr_err("read data : %d\n", charging_limit_threshold);
}
...
// Declare the is_input_present() function before using it
extern bool boost_up_voltage_done;
bool g_charging = true;
extern void demochg_judge(int en);
extern bool g_demochg_flag;
extern bool g_demochg_res;
...
static void update_sram_data(struct fg_chip *chip, int *resched_ms)
{
	...
	int prtemp, prsoc, prcapacity, prvoltage, prcurrent;
	...
	prtemp = get_sram_prop_now(chip, FG_DATA_BATT_TEMP);
	prsoc = fg_data[FG_DATA_BATT_SOC].value/100;
	prcapacity = get_prop_capacity(chip);	
	prvoltage = fg_data[FG_DATA_CPRED_VOLTAGE].value/1000;
	prcurrent = -(fg_data[FG_DATA_CURRENT].value/1000);
	charge_soc = mapping_charge[adc_temp];
	discharge_soc = mapping_discharge[adc_temp];
	
	printk(KERN_ERR "<BATT> P:%d %%(%d %%), V:%d mV, C:%d mA, T:%d (0.1C), ADC:%lld, CP: %d %%, DP: %d %%, UI: %d %%\n",
		prcapacity, prsoc, prvoltage, prcurrent, prtemp, adc_temp, charge_soc, discharge_soc,get_prop_capacity_asus(chip));
	
	/* modify polling time with capacity. shilun_huang@asus.com*/
	if (prcapacity > 50)
		fg_sram_update_period_ms = 60000;
	else if (prcapacity > 10)
...
//		if(prcapacity >= CHARGING_LIMIT_THRESHOLD)
		if (boost_up_voltage_done){
			if(b_eng_charging_limit && prcapacity >= charging_limit_threshold){
				eng_charging_limit(STOP_CHARGING_ENABLE);
			} else{
				eng_charging_limit(STOP_CHARGING_DISABLE); // When leave stop charging state to re-charging
				jeita_control( prsoc, prtemp, prvoltage);
			}
		}
	}
#else // NOT ENG_BUILD
	if(is_input_present(chip)) {
		...
		if (boost_up_voltage_done){
			if (g_demochg_res)
				pr_info("force disable charging\n");
			else
				jeita_control(prcapacity, prtemp, prvoltage);
		}
		
#if SUPPORT_DEMO_MODE_CHG_CTL
/*		if(get_demo_mode_status() == DEMO_MODE_ENABLE)
		{
			pr_info("Entry the vzw demo mode, the battery capacity is %d %%\n", prcapacity);

			if (boost_up_voltage_done && g_charging)
				jeita_control( prsoc, prtemp, prvoltage);
			
			if(prcapacity >= DEMO_MODE_CTL_MAX_CAPACITY)
			{
				// Stop Charging
				vzw_demo_mode_control(STOP_CHARGING_ENABLE);
				g_charging = false;
			}
			else if(prcapacity <= DEMO_MODE_CTL_MIN_CAPACITY)
			{
				// Re-Start Charging
				vzw_demo_mode_control(STOP_CHARGING_DISABLE);				
				g_charging = true;
			}
		}
*/
		/*else
		{
			// exit demo mode then check the charging status
			pr_info("No entry the vzw demo mode ###\n");
		}*/
	...
	/* Notify battery capacity to external */
	power_notify_capacity_device(prcapacity);
}
...
static void status_change_work(struct work_struct *work)
{
	...
	int cc_soc, rc, capacity = get_prop_capacity_asus(chip);
...
#if defined(CONFIG_ZT582KL)
	batt_node = of_find_node_by_name(node, "qcom,battery-data-zt582kl");
#elif defined(CONFIG_ZT581KL) || defined(CONFIG_Z581KL)
	batt_node = of_find_node_by_name(node, "qcom,battery-data-zt581kl");
#elif defined(CONFIG_ZT500KL)	
	batt_node = of_find_node_by_name(node, "qcom,battery-data-zt500kl");
#else
	batt_node = of_find_node_by_name(node, "qcom,battery-data-zt581kl");
#endif
	if (!batt_node) {
...
int fg_read_mem_data(int index)
{
	int value = -1;
	...
	{
		case FG_DATA_BATT_SOC:
			value = fg_data[FG_DATA_BATT_SOC].value/100;
			break;
		case FG_DATA_CPRED_VOLTAGE:
			value = fg_data[FG_DATA_CPRED_VOLTAGE].value/1000;
			break;
		default:
			break;
	}
...
EXPORT_SYMBOL(fg_read_mem_data);
...
static void delayed_init_work(struct work_struct *work)
{
	...
	if (chip->wa_flag & IADC_GAIN_COMP_WA) {
		...
		pre_usb_present = chip->input_present;
...
{
	struct fg_chip *chip = container_of(alarm, struct fg_chip, AC_charging_alarm_timer);
	wake_lock(&AC_charging_lock);
	pr_err("reStart!\n");

	if(atomic_read(&AC_charging_alarm) == 1){
		cancel_delayed_work(&chip->update_sram_data);
		schedule_delayed_work(&chip->update_sram_data,	msecs_to_jiffies(0));
		alarm_forward_now(&chip->AC_charging_alarm_timer, ns_to_ktime(60000000000)); //60s
		pr_err("End!\n");
		return ALARMTIMER_RESTART;
	}
	else{
...
/* Add by Shilun for UI battery version */
ssize_t battery_switch_name(struct switch_dev *sdev, char *buf)
{
...
static int fg_probe(struct spmi_device *spmi)
{
	...
	wake_lock_init(&AC_charging_lock, WAKE_LOCK_SUSPEND, "battery suspend wakelock");
...
#if defined(ENG_BUILD)
	INIT_DELAYED_WORK(&chip->read_charging_limit_work,read_charging_limit_from_file);
#endif
	...
	alarm_init(&chip->AC_charging_alarm_timer, ALARM_REALTIME, AC_charging_polling_atimer);
	init_completion(&chip->sram_access_granted);
	init_completion(&chip->sram_access_revoked);
	complete_all(&chip->sram_access_revoked);
	init_completion(&chip->batt_id_avail);
...
#if defined(ENG_BUILD)
	schedule_delayed_work(&chip->read_charging_limit_work,msecs_to_jiffies(2000));
#endif
	...
	pr_info("create sdev for battery ...\n");
	chip->battery_sdev.name = "battery";
	chip->battery_sdev.print_name = battery_switch_name;
	if(switch_dev_register(&chip->battery_sdev) < 0)
		pr_err("switch_dev_register battery failed!\n");
...
extern void get_usb_type(enum power_supply_type *usb_supply_type);
static int fg_prepare(struct device *dev)
{
	struct fg_chip *chip = dev_get_drvdata(dev);
	...
	if( usb_supply_type == POWER_SUPPLY_TYPE_USB_DCP||
		...
		usb_supply_type == POWER_SUPPLY_TYPE_USB_HVDCP_3)
	{
		pr_err("Charging Type = %d\n", usb_supply_type);
		if (atomic_read(&AC_charging_alarm) == 0)
		{
			alarm_start_relative(&chip->AC_charging_alarm_timer, ns_to_ktime(60000000000)); //60s
			pr_err("Set Wake_BAT_alarm_timer = 60\n");
