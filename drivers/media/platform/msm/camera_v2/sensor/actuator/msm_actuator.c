...
void asus_actuator_move (int position){
       ...
       pr_err("[ASUS_VCM] vcm move position: %d\n",position);
...
//ASUS_BSP+++ manual VCM move
static uint16_t vcm_manual_steps = 1024;
#define CAMERA_PROC_VCM_FILE "vcm"
static struct proc_dir_entry *camera_proc_vcm_file;

static int camera_proc_vcm_init(struct seq_file *buf, void *v){
       pr_info("camera_proc_read vcm_steps %d\n", vcm_steps);
       seq_printf(buf, "%d\n", vcm_steps);
       return 0;
}

static int camera_proc_vcm_open(struct inode *inode, struct  file *file) {
    return single_open(file, camera_proc_vcm_init, NULL);
}

static ssize_t write_vcm_step(struct file *file,
                const char __user *user_buf, size_t count, loff_t *ppos)
{
	sscanf(user_buf, "%hd", &vcm_manual_steps);
        if (vcm_manual_steps < 0 || vcm_manual_steps > 1023) {
                printk("[ASUS_VCM] Illegal VCM step value : %hd, return to AutoFocus\n",vcm_manual_steps);
        } else {
                u16 addr, value;
                printk("[ASUS_VCM] Set VCM setp to : %hd\n",vcm_manual_steps);
                addr=vcm_manual_steps >>4;
                value=(vcm_manual_steps &0x0f ) <<4;
                vcm_ctrl->i2c_client.i2c_func_tbl->i2c_write(&vcm_ctrl->i2c_client,addr,value,MSM_CAMERA_I2C_BYTE_DATA);
        }
       vcm_steps = vcm_manual_steps;
        return count;
}

static const struct file_operations camera_vcm_fops = {
        .owner = THIS_MODULE,
        .open = camera_proc_vcm_open,
        .read = seq_read,
        .write = write_vcm_step,
};

void create_camera_vcm_proc_file(void){
        camera_proc_vcm_file =  proc_create(CAMERA_PROC_VCM_FILE, 0666, NULL, &camera_vcm_fops);
        if(camera_proc_vcm_file){
                printk("[ASUS_VCM] proc vcm_file create sucessed!\n");
        }
	vcm_manual_steps = 1024;
}

/*void remove_camera_vcm_proc_file(void)
{
	remove_proc_entry(CAMERA_PROC_VCM_FILE, NULL);
	vcm_manual_steps = 1024;
}*/
//ASUS_BSP--- manual VCM move
	...
	struct msm_actuator_set_position_t *set_pos)
{
	...
	//ASUS_BSP+++ 
	if (vcm_manual_steps >= 0 && vcm_manual_steps < 1024) {
		vcm_steps = vcm_manual_steps;
		return 0;
	}
	//ASUS_BSP---
...
static int32_t msm_actuator_platform_probe(struct platform_device *pdev)
{
	...
	msm_actuator_t->msm_sd.sd.devnode->fops =
      ...
      create_camera_vcm_proc_file(); //ASUS_BSP+++ manual VCM move
	CDBG("Exit\n");
	return rc;
}
