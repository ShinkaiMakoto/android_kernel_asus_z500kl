...
uint16_t isZT582KL = 1;
#endif

// For DIT VCM debug Interface +++
#if (defined(CONFIG_ZT500KL) || defined(CONFIG_Z500KL) || defined(CONFIG_ZT582KL))
#define CAMERA_PROC_OTP_FILE "rear_otp"
#define CAMERA_PROC_OTP_UNIQUE_ID_FILE "rear_otp_uid"
#else
...
static uint8_t front_otp_data[32];
uint16_t front_total_24 = 0, front_total_32 = 0, front_check_sum_24 = 0, front_check_sum_32 = 0;

// For DIT VCM debug Interface +++
#define CAMERA_PROC_FRONT_OTP_FILE "front_otp"
#define CAMERA_PROC_FRONT_OTP_UNIQUE_ID_FILE "front_otp_uid"

static struct proc_dir_entry *camera_proc_front_otp_file;
static struct proc_dir_entry *camera_proc_front_otp_uid_file;

static int camera_proc_front_otp_read(struct seq_file *buf, void *v){
        int i;
#if defined(CONFIG_ZT582KL)
	if (!isZT582KL) {
#else
        if (front_total_32 != front_check_sum_32) {
#endif
                for (i = 0;i < 24;i++){
                    seq_printf(buf, "0x%X", front_otp_data[i]);
                        if((i+1) % 8 != 0)
                                seq_printf(buf, " ");
                          else
                                seq_printf(buf, "\n");
...
static int camera_proc_front_otp_open(struct inode *inode, struct  file *file) {
    return single_open(file, camera_proc_front_otp_read, NULL);
}

static const struct file_operations camera_front_otp_fops = {
        .owner = THIS_MODULE,
        .open = camera_proc_front_otp_open,
        .read = seq_read,
};

static int camera_proc_front_otp_uid_read(struct seq_file *buf, void *v){
        int i;

        for (i = 10;i < 22;i++)
                seq_printf(buf, "%X", front_otp_data[i]);
        seq_printf(buf, "\n");
        return 0;
}

static int camera_proc_front_otp_uid_open(struct inode *inode, struct  file *file) {
    return single_open(file, camera_proc_front_otp_uid_read, NULL);
}

static const struct file_operations camera_front_otp_uid_fops = {
        .owner = THIS_MODULE,
        .open = camera_proc_front_otp_uid_open,
        .read = seq_read,
};

void create_camera_front_proc_file(void){

        camera_proc_front_otp_file = proc_create(CAMERA_PROC_FRONT_OTP_FILE, 0666, NULL, &camera_front_otp_fops);
        if(camera_proc_front_otp_file){
                printk("proc front_otp file create sucessed!\n");
        }
        else{
                printk("proc front_otp file create failed!\n");
        }
        camera_proc_front_otp_uid_file = proc_create(CAMERA_PROC_FRONT_OTP_UNIQUE_ID_FILE, 0666, NULL, &camera_front_otp_uid_fops);
        if(camera_proc_front_otp_uid_file){
                printk("proc front_otp_uid file create sucessed!\n");
        }
        else{
                printk("proc front_otp_uid file create failed!\n");
        }
}

void remove_camera_front_proc_file(void)
{
    remove_proc_entry(CAMERA_PROC_FRONT_OTP_FILE, NULL);
    remove_proc_entry(CAMERA_PROC_FRONT_OTP_UNIQUE_ID_FILE, NULL);
}
...
//ASUS_BSP+++
#ifdef CONFIG_ZT582KL
	if (strcmp("ov13850",sensor_name) == 0) {
		rc = sensor_i2c_client->i2c_func_tbl->i2c_read(
			sensor_i2c_client, 0x302A,
			&revision_id, MSM_CAMERA_I2C_WORD_DATA);
		if (rc < 0) {
			pr_err("%s: %s: read revision id failed\n", __func__, sensor_name);
			return rc;
		}
		revision_id = revision_id >> 8;
		pr_err("%s: read id: 0x%x\n", __func__, revision_id);
		if (revision_id != 0xB2) {
			pr_err("%s: read id: 0x%x expected id 0xB2:\n", __func__, revision_id);
			pr_err("[ZT582KL] Probed %s fail: Sensor is not R2A version\n",sensor_name);
			return -ENODEV;
		}
	}
#endif
...
int msm_sensor_read_otp(struct msm_sensor_ctrl_t *s_ctrl){
...
#if (defined(CONFIG_ZT500KL) || defined(CONFIG_Z500KL) || defined(CONFIG_ZT582KL))
        if(strcmp("ov5670",sensor_name) == 0 || strcmp("ov8856",sensor_name) == 0 ||
           strcmp("ov5670_polaris",sensor_name) == 0 || strcmp("ov13850",sensor_name) == 0) {
                int start_addr, end_addr;
		uint16_t local_data;

                pr_err("[ZT500KL/Z500KL/ZT582KL] Camera %s\n", sensor_name);
			...
			if (strcmp("ov13850",sensor_name) == 0) {
                        ...
                        for(i = 0; i < 30; i++){
                                front_total_32+=read_value[i];
                        }

                        front_check_sum_32 = (read_value[30] << 8) + read_value [31];
				...
				if(front_total_32 == front_check_sum_32){
				    ...
				    if(strcmp("ov8856",sensor_name) == 0 || strcmp("ov13850",sensor_name) == 0) {
					    create_camera_proc_file();
				    } else {
					    create_camera_front_proc_file();
				    }
                                    goto out;
                                }
                                else{
	...
	if(strcmp("t4k35",sensor_name) == 0 || strcmp("t4k35_4lane",sensor_name) == 0){
	    uint16_t data;
...
#if (defined(CONFIG_ZT500KL) || defined(CONFIG_Z500KL) || defined(CONFIG_ZT582KL))
	if (strcmp("ov5670",sensor_name) == 0 || strcmp("ov5670_polaris",sensor_name) == 0) {
        	for(i = 0; i < 32; i++)
                	front_otp_data[i] = (uint8_t) read_value[i];
...
//ASUS_BSP+++ Himax [HM2051] Patch do not use burst mode
                if (CFG_WRITE_I2C_ARRAY == cdata->cfgtype) {
                        if (strcmp(s_ctrl->sensordata->sensor_name, "hm2051") == 0) {
				conf_array.data_type = MSM_CAMERA_I2C_BYTE_DATA;
                                if(conf_array.reg_setting[0].reg_addr == 0x34c || conf_array.reg_setting[0].reg_addr == 0x0104) {
                                        pr_err("%s:%d skip 0x%x command\n", __func__, __LINE__, conf_array.reg_setting[0].reg_addr);
                                        rc = 0;
                                        msleep(20);
                                } else {
                                        num = conf_array.size;
...
static struct msm_sensor_fn_t msm_sensor_func_tbl = {
	...
	.sensor_read_otp = msm_sensor_read_otp,//ASUS_BSP+++
};
