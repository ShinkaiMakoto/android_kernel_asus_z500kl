...
#if GTP_DEBUG_ON
//static const int  key_codes[] = {KEY_HOME, KEY_BACK, KEY_APP_SWITCH, KEY_SEARCH};
//static const char *key_names[] = {"Key_Home", "Key_Back", "Key_App_Switch", "Key_Search"};
static const int  key_codes[] = {KEY_HOME, KEY_BACK, KEY_APP_SWITCH};
static const char *key_names[] = {"Key_Home", "Key_Back", "Key_App_Switch"};
#endif
...
// add by leo ++
int gt6108_lcm_source = 0;
extern int Read_HW_ID(void);
extern int Read_second_source_LCM(void);
// add by leo --

// add by alp.D ++
int g_gt6108_i2c_state = 1;
// add by alp.D --
        ...
        if (ret > 0)
        {
              ...
              )
            {
                ...
                if((doze_buf[2] == 'w') && (ts->gesture & GESTURE_W))
                {
                    ...
                    input_sync(ts->input_dev);
                    printk(KERN_ERR "alp.D : gesture w !!\n");
                }
                else if((doze_buf[2] == 's') && (ts->gesture & GESTURE_S))
                {
                    ...
                    input_sync(ts->input_dev);
                    printk(KERN_ERR "alp.D : gesture s !!\n");
                }
                else if((doze_buf[2] == 'e') && (ts->gesture & GESTURE_E))
                {
                    ...
                    input_sync(ts->input_dev);
                    printk(KERN_ERR "alp.D : gesture e !!\n");
                }
                else if((doze_buf[2] == 'c') && (ts->gesture & GESTURE_C))
                {
                    ...
                    input_sync(ts->input_dev);
                    printk(KERN_ERR "alp.D : gesture c !!\n");
                }
                else if((doze_buf[2] == 'z') && (ts->gesture & GESTURE_Z))
                {
                    ...
                    input_sync(ts->input_dev);
                    printk(KERN_ERR "alp.D : gesture z !!\n");
                }
                else if((doze_buf[2] == 'v') && (ts->gesture & GESTURE_V))
                {
                    ...
                    input_sync(ts->input_dev);
                    printk(KERN_ERR "alp.D : gesture v !!\n");
                }
                /*
            ...
            else if((0xCC == doze_buf[2]) && (ts->gesture & GESTURE_DOUBLE_CLICK))  // double click
            {
                ...
                input_sync(ts->input_dev);
                printk(KERN_ERR "alp.D : gesture double click !!");
#endif
        ...
        if (!pre_touch)
        {
            for (i = 0; i < GTP_MAX_KEY_NUM; i++)
            {
                ...
                for (ret = 0; ret < 4; ++ret)
                {
                    ...
                    {
                        GTP_DEBUG("Key(%d): %s %s", touch_key_array[i], key_names[ret], (key_value & (0x01 << i)) ? "Down" : "Up");
                        break;
                    }
                }
#endif
...
static int gtp_check_product_id(struct i2c_client *client)
{
    ...
    dev_info(&client->dev, "ts->pdata->product_id = %s\n", ts->pdata->product_id);
                           ...
                           struct goodix_ts_platform_data *pdata)
{
    ...
    {
	printk("[%s] of_get_named_gpio_flags goodix,reset-gpio fail", __func__); // add by leo
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(ret < 0)
        printk(KERN_ERR "[elan] %s:Failed to request GPIO%d (goodix_ts_power_gpio) ret=%d\n", __func__, pdata->power_gpio, ret);
	...
	if(g_gt6108_i2c_state==0){
        return 0 ;
        GTP_ERROR("alp.D i2c test fail , return!!!");
	}
	...
	printk("alp.D : gtp_request_irq irq=%d\n", ts->use_irq);
...
static int __init goodix_ts_init(void)
{
    ...
    GTP_DEBUG_FUNC();
    gt6108_lcm_source = 1; //Read_second_source_LCM();
    GTP_INFO("gt6108_lcm_source = %d\n", gt6108_lcm_source);
    if (gt6108_lcm_source == 1)
    {
    ...
    {
        GTP_INFO("skip GTP driver instal");
    }
    return ret;
}
