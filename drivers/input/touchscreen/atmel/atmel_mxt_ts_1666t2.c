...
static ssize_t mxt_proc_stylus_write(struct file *filp, const char __user *buf, size_t len, loff_t *data)
{
	if (buf[0]=='0'){
		...
		/* Modify by Tom Cheng for ZT500KL Touch Config Setting */
		mxt_set_t8_acquisition_cfg(touch_chip, 10,
			(MXT_MEASALLOW_MUTUALTCH | MXT_MEASALLOW_SELFTCH | MXT_MEASALLOW_SELFPROX));
...
static void mxt_start(struct mxt_data *data)
{
	...
	if (data->use_regulator) {
		...
		mxt_set_t8_acquisition_cfg(data, 10, data->t8_cfg.byte10); // disable Self Cap Prox by Josh_sh
