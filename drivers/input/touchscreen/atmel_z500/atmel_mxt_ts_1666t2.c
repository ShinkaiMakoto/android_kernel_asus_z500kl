...
#define MXT_PROCI_Selfcap_Measure_Config_T113	113
#define MXT_PROC_SYMBOLGESTUREPROCESSOR_T92 92
#define MXT_PROCI_TOUCHSEQUENCELOGGER_T93   93

/* MXT T15 status */
#define MXT_T15_ENABLE 		1
#define MXT_T15_DISABLE 	2

/* MXT T15 Configuration */
#define MXT_T15_CTRL_ENABLE  (1 << 0)
#define MXT_T15_CTRL_RPTEN   (1 << 1)
...
/* Add by Tom Cheng to Filter Edge Noise for Gesture Function in Suspend */
#define MXT_T40_ENABLE		1
#define MXT_T40_DISABLE		0
#define MXT_T40_GRIP_ENABLE 	(1 << 0)
...
#define MXT_PROC_TOUCH_AUTO_TEST		"touch_auto_test"
static struct proc_dir_entry *mxt_proc_touch_auto_test = NULL;
...
static bool touch_probe_status = true;
...
static u8 mxt_get_t8_cfg(struct mxt_data *data, u16 cmd_offset);
/* defined but not used
static int mxt_get_t104_cfg(struct mxt_data *data);
*/
//static int mxt_get_t107_cfg(struct mxt_data *data, u16 cmd_offset);
static int mxt_get_t113_cfg(struct mxt_data *data, u16 cmd_offset);
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	if(data->debug_log_enabled == 3){
		LDBG("id= %d, status= 0x%x type=%d\n",id,(status & MXT_T100_DETECT), ((status & MXT_T100_TYPE_MASK) >> 4));
	}
	if (status & MXT_T100_DETECT) {
		...
		switch (type) {
			...
			case MXT_T100_TYPE_ACTIVE_STYLUS:
				if(touch_chip->z_stylus_only_mode == 1 && have_stylus == 0){
                    have_stylus = 1;
	...
	if(active){	
		if (type == MXT_T100_TYPE_ACTIVE_STYLUS){
			...
			//batlvl--
			//LDBG("active_pen_mode= %d, current_batlvl= %d, batlvl= %d\n",active_pen_mode,current_batlvl,batlvl);
...
static void mxt_proc_t15_messages(struct mxt_data *data, u8 *msg)
{
	...
	for (key = 0; key < 2; key++) {
		...
		if (!curr_state && new_state) {
			...
			if (key == 0) {
				LDBG("T15 KEY_BACK press\n");
				input_report_key(input_dev, KEY_BACK, 1);
			}
			if (key == 1) {
				LDBG("T15 APP_SWITCH press\n");
			...
			if (key == 0) {
				LDBG("T15 KEY_BACK release\n");
				input_report_key(input_dev, KEY_BACK, 0);
			}
			if (key == 1) {
				LDBG("T15 APP_SWITCH release\n");
...
static int mxt_t15_configuration(struct mxt_data *data, u16 cmd_offset,
				u8 status)
{
	u16 reg;
	u8 command_register;
	...
	reg = data->T15_address + cmd_offset;

	ret = __mxt_read_reg(data->client, reg, 1, &command_register);
	if (ret)
		return ret;

	if (status == MXT_T15_ENABLE)
		command_register |= (MXT_T15_CTRL_ENABLE|MXT_T15_CTRL_RPTEN);
	if (status == MXT_T15_DISABLE)
		command_register &= ~(MXT_T15_CTRL_ENABLE|MXT_T15_CTRL_RPTEN);
				  ...
				  struct mxt_object *object_table)
{
	...
	for (i = 0; i < data->info->object_num; i++) {
		...
		LDBG("T%u Start:%u Size:%zu Instances:%zu Report IDs:%u-%u\n",
		...
		case MXT_PROCI_ACTIVESTYLUS_T107:
			...
			break;	
		case MXT_PROCI_Selfcap_Measure_Config_T113:
					...
					u16 cmd_offset, u8 type)
{
	...
	LDBG("mxt_set_t8_acquisition_cfg :%d \n",type);
...
recheck:
	...
	data->t8_cfg.byte2=mxt_get_t8_cfg(data,2);
	data->t8_cfg.byte3=mxt_get_t8_cfg(data,3);
	data->t8_cfg.byte10=mxt_get_t8_cfg(data,10);
	LDBG("byte2: %d, byte3: %d, byte10: %d\n",data->t8_cfg.byte2,data->t8_cfg.byte3,data->t8_cfg.byte10);
	//mxt_get_t104_cfg(data);
	//mxt_get_t107_cfg(data, 97);
	mxt_get_t113_cfg(data, 0);
...
static u8 mxt_get_t8_cfg(struct mxt_data *data, u16 cmd_offset)
{
	int error;
	u8 temp;
	error = __mxt_read_reg(data->client, data->T8_address + cmd_offset,
				sizeof(temp), &temp);
	if (error)
		return error;
...
/* Add by Tom Cheng to Filter Edge Noise for Gesture Function in Suspend (T40) 
static int mxt_get_t40_cfg(struct mxt_data *data)
{
	int error;

	error = __mxt_read_reg(data->client, data->T40_address,
				sizeof(data->t40_cfg), &data->t40_cfg);
	if (error)
		return error;
	
	LDBG("[Atmel] \n [Atmel]byte0: %d\n [Atmel]byte1: %d\n [Atmel]byte2: %d\n [Atmel]byte3: %d\n [Atmel]byte4: %d\n",
		data->t40_cfg.byte0, data->t40_cfg.byte1, data->t40_cfg.byte2, data->t40_cfg.byte3,	data->t40_cfg.byte4);
	return 0;
} */
 
static int mxt_set_t40_grio_cfg(struct mxt_data *data, u8 type)
{
	struct t40_config *now_config ;
	struct t40_config old_config = data->t40_cfg;
        int error;
	...
	if (type == MXT_T40_ENABLE) {
		old_config.byte0 = 17;
		old_config.byte1 = 255;
		old_config.byte2 = 255;
		old_config.byte3 = 255;
		old_config.byte4 = 255;
		now_config = &old_config;
   	} else if (type == MXT_T40_DISABLE) {
		old_config.byte0 = 16;
		old_config.byte1 = 255;
		old_config.byte2 = 255;
		old_config.byte3 = 255;
	...
	error = __mxt_write_reg(data->client, data->T40_address,
				sizeof(data->t40_cfg), now_config);
	if (error)
		return error;
...
static int mxt_get_t113_cfg(struct mxt_data *data,u16 cmd_offset)
{
	...
	error = __mxt_read_reg(data->client, data->T113_address + cmd_offset,
				sizeof(data->t113_cfg), &data->t113_cfg);
	if (error)
		return error;
...
static int mxt_set_t113_cfg(struct mxt_data *data,
					u16 cmd_offset, u8 type)
{
	u16 reg;
    int ret;
	LDBG("mxt_set_t113_cfg :%d \n",type);
	if(build_version == 1)
		return 0;

    reg = data->T113_address + cmd_offset;

    ret = mxt_write_reg(data->client, reg, type);
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	printk("[Ateml]: %u.%u.%02X-%u.%u-%06x\n",
			 chip_fw_version >> 4, chip_fw_version & 0xf,
			 chip_fw_build,data->maj_version,data->min_version,touch_chip->config_crc);
	
	return scnprintf(buf, PAGE_SIZE, "%u.%u.%02X\n",
			 chip_fw_version >> 4, chip_fw_version & 0xf, chip_fw_build);
}
...
static ssize_t mxt_proc_stylus_write(struct file *filp, const char __user *buf, size_t nbytes, loff_t *offset)
{
	if (buf[0]=='0'){
		touch_chip->z_stylus = 0;
		printk("%s: ATMEL TEST1 z_stylus = %d\n", __func__,touch_chip->z_stylus);
		...
		printk("%s: ATMEL TEST1 z_stylus = %d\n", __func__,touch_chip->z_stylus);		
		mxt_set_t8_acquisition_cfg(touch_chip, 10,
			(MXT_MEASALLOW_MUTUALTCH | MXT_MEASALLOW_SELFTCH | MXT_MEASALLOW_ACTVSTY | MXT_MEASALLOW_SELFPROX));
...
#ifdef mxt_zstylus_only_mode
static int mxt_proc_zstylus_only_mode_read(struct seq_file *buf, void *v)
{
       printk("%s: z_stylus_only_mode = %d, second = %d!!!!!!\n", __func__,touch_chip->z_stylus_only_mode,touch_chip->z_stylus_only_mode_second);
       return 0;
}

static ssize_t mxt_proc_zstylus_only_mode_write(struct file *filp, const char __user *buf, size_t nbytes, loff_t *offset)
{
       if (buf[0]=='0'){
...
void mxt_create_proc_zstylus_only_mode_file(void)
{
...
void mxt_remove_proc_zstylus_only_mode_file(void)
{
       extern struct proc_dir_entry proc_root;
...
{
	mxt_proc_logtool_file = proc_create(MXT_PROC_LOGTOOL_FILE, 0666, NULL, &mxt_logtool_fops);
	if(mxt_proc_logtool_file)
	{
...
static int mxt_proc_touch_auto_test_read(struct seq_file *buf, void *v)
{
       touch_probe_status = false;
	   printk("%s: touch_probe_status = %d!!!!!!\n", __func__,touch_probe_status);
       return seq_printf(buf,"%d\n",touch_probe_status);
}

static ssize_t mxt_proc_touch_auto_test_write(struct file *filp, const char __user *buf, size_t nbytes, loff_t *offset)
{
       return nbytes;
}

static int mxt_proc_touch_auto_test_open(struct inode *inode, struct  file *file)
{
       return single_open(file, mxt_proc_touch_auto_test_read, NULL);
}

static const struct file_operations mxt_touch_auto_test_fops =
{
       .owner = THIS_MODULE,
       .open = mxt_proc_touch_auto_test_open,
       .read = seq_read,
       .write = mxt_proc_touch_auto_test_write,
};

void mxt_create_proc_touch_auto_test(void)
{
       mxt_proc_touch_auto_test = proc_create(MXT_PROC_TOUCH_AUTO_TEST, 0666, NULL, &mxt_touch_auto_test_fops);
       if(mxt_proc_touch_auto_test)
       {
               printk("%s:: proc touch_auto_test create sucessed\n", __func__);
       }
       else
       {
               printk("%s:[%d]: proc touch_auto_test create failed\n", __func__, __LINE__);
       }
       return;
}
void mxt_remove_proc_touch_auto_test(void)
{
       extern struct proc_dir_entry proc_root;
       printk("%s:[%d]: proc touch_auto_test removed.", __func__, __LINE__);
       remove_proc_entry(MXT_PROC_TOUCH_AUTO_TEST, &proc_root);
       return;
}

static void function_key_status(struct mxt_data *data, u8 status)
{
	/* Disable function Key*/
	if (data->T15_address) {
		mxt_t15_configuration(data, 0, status);
	}
}
...
static void mxt_start(struct mxt_data *data)
{
	...
	if (data->use_regulator) {
		...
		mxt_set_t8_acquisition_cfg(data, 2, data->t8_cfg.byte2); 
		mxt_set_t8_acquisition_cfg(data, 3, data->t8_cfg.byte3); 
		LDBG(" TEST. %d.%d\n", data->z_stylus, (data->t8_cfg.byte10 | MXT_MEASALLOW_ACTVSTY));
		
		// disable Self Cap and enable stylus Prox by Josh_sh
		if(data->z_stylus == 1){
			mxt_set_t8_acquisition_cfg(data, 10, (data->t8_cfg.byte10 | MXT_MEASALLOW_ACTVSTY)); 
		} else {
			mxt_set_t8_acquisition_cfg(data, 10, (data->t8_cfg.byte10 | (!MXT_MEASALLOW_ACTVSTY))); 
		}
		...
		mxt_set_t113_cfg(data, 0, data->t113_cfg.byte0); //For gesture power issue by Josh_sh
		...
		function_key_status(data,MXT_T15_ENABLE);
	...
	if((data->double_tap_enable == 0 && data->gesture_enable == 0) || (build_version == 1) || (hall_status == true)){
		...
		/* Add by Tom Cheng to Filter Edge Noise for Gesture Function in Suspend */
		mxt_set_t40_grio_cfg(data, MXT_T40_DISABLE);
	}
	mxt_irq_enable(data->irq);
...
static void mxt_stop(struct mxt_data *data)
{
	...
	if (data->use_regulator){
		...
		if((data->double_tap_enable == 0 && data->gesture_enable == 0) || (build_version == 1)  || (hall_trigger_suspend == true)){
			...
			/* Add by Tom Cheng to Filter Edge Noise for Gesture Function in Suspend */
			mxt_set_t8_acquisition_cfg(data, 2, 10); // disable Self Cap Prox by Josh_sh
			mxt_set_t8_acquisition_cfg(data, 3, 10); // disable Self Cap Prox by Josh_sh
			mxt_set_t8_acquisition_cfg(data, 10, 3); // disable Self Cap Prox by Josh_sh
			mxt_set_t113_cfg(data, 0, 0); //For gesture power issue by Josh_sh
			mxt_set_t40_grio_cfg(data, MXT_T40_ENABLE);
			mxt_wake_irq_enable(data->irq);
			LDBG(" ATMEL Touch GESTURE ... \n");
		}


		// Mark by Tom Cheng for Disable stylus for ZT500KL
		//disable_stylus(data);
		/*if(data->z_stylus == 1)
			mxt_t107_configuration(touch_chip, 0, MXT_T107_DISABLE);*/
		
		function_key_status(data,MXT_T15_DISABLE);
...
/*
 * Mark by Tom Cheng for warning: 'mxt_touch_sysfs_init' defined but not used 
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	mxt_create_proc_touch_auto_test();
	...
	/* Add by Tom Cheng for Check is entry_mode */
	if(entry_mode != 1) {
		msleep(1000); //add dely time to fix cos hang by tom (time issue)
		// Add by Tom Cheng for enter suspend mode
		//touch_ic_status = true;
		//mxt_suspend(&data->client->dev);
		//LDBG(" Not in MOS, skip probe. entry_mode : [%d] ,touch_ic_status [%d]\n",entry_mode, touch_ic_status);
		// Add by Tom Cheng for disable touch_ic_status in COS
		mxt_set_t7_power_cfg(data, MXT_POWER_CFG_DEEPSLEEP);		
		LDBG(" ATMEL Touch DEEPSLEEP ... \n");
		touch_ic_status = false;
		//LDBG(" touch_ic_status [%d]\n", touch_ic_status);
		return 0;
	}
...
static int mxt_resume(struct device *dev)
{
	...
	if (!touch_chip->suspended){
		LDBG("Already in awake state\n");
		return 0;
	}
