...
void fts_get_upgrade_array(void)
{
    ...
    // add by leo for CHIP_ID issue ++
    printk("[ft5726] %s register chip_id = 0x%02X\n", __func__, chip_id);
    chip_id = 0x58;
    printk("[ft5726] %s directly write chip_id = 0x%02X\n", __func__, chip_id);
    // add by leo for CHIP_ID issue --
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
    ...
    printk("[ft5726] %s: START ... fwsize: %d\n", __func__,fwsize); // add by leo for testtest
    if (fwsize <= 0)
    {
    ...
    printk("[ft5726] %s: fts_updateinfo_curr.CHIP_ID = 0x%x\n", __func__,fts_updateinfo_curr.CHIP_ID); // add by leo for testtest
    printk("[ft5726] %s: fts_5822_ctpm_fw_upgrade\n", __func__); // add by leo for testtest
...
int  fts_ctpm_fw_preupgrade_hwreset(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
        printk("[ft5726] %s: ID1 = 0x%x,ID2 = 0x%x\n", __func__, reg_val[0], reg_val[1]); // add by leo for testtest
...
int fts_ctpm_fw_download(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        printk("[FTS] Download Step 2:Enter upgrade mode \n");
        /*********Step 2:Enter upgrade mode *****/
    ...
    printk("[FTS] Step zaxzaxzaxzax Download Step change to write flash mode\n");
    if (i >= FTS_UPGRADE_LOOP)
    {
        printk("[FTS] Step z11111axzaxzaxzax Download Step change to write flash mode\n");
        if (reg_val[0] != 0x00 || reg_val[1] != 0x00)
        {
            printk("[FTS] Step zax333333zaxzaxzax Download Step change to write flash mode\n");
            if (gpio_is_valid(private_ts->pdata->reset_gpio))
            {
                gpio_set_value_cansleep(private_ts->pdata->reset_gpio, 0);
                mdelay(10);
        ...
        printk("[FTS] Step zaxza222222xzaxzax Download Step change to write flash mode\n");
        return -EIO;
    }

    printk("[FTS] Download Step 4:change to write flash mode\n");
    fts_write_reg(client, 0x09, 0x0a);

    /*Step 4:erase app and panel paramenter area*/
    printk("[FTS] Download Step 4:erase app and panel paramenter area\n");
    auc_i2c_write_buf[0] = 0x61;
    fts_i2c_write(client, auc_i2c_write_buf, 1);	/*erase app area */
    ...
    printk("[FTS] Download Step 5:write firmware(FW) to ctpm flash\n");
    /*********Step 5:write firmware(FW) to ctpm flash*********/
    bt_ecc = 0;
    ...
    printk("[FTS] Download Step 6: read out checksum\n");
    /*********Step 6: read out checksum***********************/
    auc_i2c_write_buf[0] = 0x64;
    fts_i2c_write(client, auc_i2c_write_buf, 1);
    mdelay(300);
    ...
    if(i_ret == 0)
    {
        printk("HidI2c change to StdI2c i_ret = %d ! \n", i_ret);
    }
...
int fts_ctpm_fw_download_with_i_file(struct i2c_client *client, char *firmware_name)
{
    ...
    if (i_ret != 0)
    {
        printk( "%s: Step FTS write pram failed. err.\n",
                __func__);
    ...
    if (i_ret != 0)
    {
        printk( "%s: Step FTS upgrade failed. err.\n",
                __func__);
        return -EIO;
    }
