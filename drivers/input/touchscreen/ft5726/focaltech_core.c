...
#define TOUCH_04MM_TP_DEFINE 0x80 // < 0x80: 0.55mm TP, > 0x80: 0.4mm TP
#define TOUCH_SDEV_NAME "touch"
#define boolean unsigned char
#define TOUCH_FW_PATH "touch_fw_app.bin"
#define TOUCH_055MM_FW_GIS_PATH "touch_055mm_fw_gis_app.bin"
#define TOUCH_055MM_FW_BIEL_PATH "touch_055mm_fw_biel_app.bin"
#define TOUCH_FW_TIANMA_GIS_PATH "touch_fw_tm_gis_app.bin"
#define TOUCH_FW_TIANMA_BIEL_PATH "touch_fw_tm_biel_app.bin"
#define TOUCH_FW_SHARP_GIS_PATH "touch_fw_sharp_gis_app.bin"
#define TOUCH_FW_SHARP_BIEL_PATH "touch_fw_sharp_biel_app.bin"

void ft5726_touch_power_enable(void);
...
static int irq_is_disable = -1;
static int usb_state = 0;
int FT5726_HW_ID = 0;
int FT5726_TP_ID = 0;
int FT5726_LCM_ID = 0;
int FT5726_PROJ_ID = 0;
int FW_VERSION = 0;
...
*  Return:
*******************************************************************************/
static int pressure_workaround = 0; // add by leo for pressure workaround
static void fts_report_value(struct fts_ts_data *data)
{
    ...
    for (i = 0; i < event->touch_point; i++)
    {
        ...
        if (event->au8_touch_event[i] == FTS_TOUCH_DOWN || event->au8_touch_event[i] == FTS_TOUCH_CONTACT)
        {
            ...
            // add by leo for pressure workaround ++
            if(pressure_workaround == 21)
                pressure_workaround = 20;
            else
                pressure_workaround = 21;
            input_report_abs(data->input_dev, ABS_MT_PRESSURE, pressure_workaround);
            ...
            if(ft5726_debug)printk("[ft5726] %s: area = %d, pressure_workaround = %d, x = %d, y = %d\n", __func__,
                                       event->area[i], pressure_workaround, event->au16_x[i], event->au16_y[i]);
        }
        else
        {
            uppoint++;
...
static void fts_touch_irq_work(struct work_struct *work)
{
    ...
    wake_lock_timeout(&private_ts->wakelock, 3 * HZ);
...
static int fts_ts_start(struct device *dev)
{
    ...
    if(err)
    {
        printk(KERN_ERR "[ft5726] %s: Failed to set reset_gpio direction low, error=%d\n", __func__, err);
        gpio_free(data->pdata->reset_gpio);
    }
    ...
    if(err)
    {
        printk(KERN_ERR "[ft5726] %s: Failed to set power_gpio direction high, error=%d\n", __func__, err);
...
static int fts_ts_stop(struct device *dev)
{
    ...
    printk("[ft5726] %s: enter sleep mode\n", __func__);
    ...
    // add by leo ++
    err = gpio_direction_output(data->pdata->power_gpio, 0);
    if(err)
    {
        printk(KERN_ERR "[ft5726] %s: Failed to set power_gpio direction high, error = %d\n", __func__, err);
        gpio_free(data->pdata->power_gpio);
...
int fts_ts_suspend(struct device *dev)
{
    ...
    if (data->suspended)
    {
        printk("[ft5726] %s: already in suspend state\n", __func__);
        return 0;
    }

    printk("[ft5726] %s: enter SUSPEND function\n", __func__);
    ...
    if(private_ts->gesture & _GESTURE_ENABLE)
    {
        ...
        data->suspended = true;
        printk("[ft5726] %s: enter gesture mode, CHIP_ID = 0x%02X\n", __func__, fts_updateinfo_curr.CHIP_ID);
        return 0;
    }
#endif
    if (data->loading_fw)
...
int fts_ts_resume(struct device *dev)
{
    ...
    if (!data->suspended)
    {
        printk("[ft5726] %s: already in awake state\n", __func__);
        return 0;
    }

    printk("[ft5726] %s: enter RESUME function\n", __func__);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    printk("[ft5726] %s: pdata->power_gpio=%d \n", __func__, pdata->power_gpio);
    ...
    printk("[ft5726] %s: pdata->tpid_gpio=%d \n", __func__, pdata->tpid_gpio);

    pdata->lcmid_gpio = of_get_named_gpio_flags(np, "focaltech,lcmid-gpio", 0, &pdata->lcmid_gpio_flags);
    if(pdata->lcmid_gpio < 0)
        return pdata->lcmid_gpio;
    printk("[ft5726] %s: pdata->lcmid_gpio=%d \n", __func__, pdata->lcmid_gpio);
    // add by leo for testtest --
    ...
    printk("[ft5726] %s: pdata->reset_gpio=%d \n", __func__, pdata->reset_gpio);

    pdata->irq_gpio = of_get_named_gpio_flags(np, "focaltech,irq-gpio", 0, &pdata->irq_gpio_flags);
    if (pdata->irq_gpio < 0)
        return pdata->irq_gpio;
    printk("[ft5726] %s: pdata->irq_gpio=%d \n", __func__, pdata->irq_gpio);
...
// add by leo ++
static void ft5726_suspend_check(struct work_struct *work)
{
    printk("[ft5726] %s: private_ts->backlight_zero = %s\n", __func__, private_ts->backlight_zero==true?"true":"false");
    if(!private_ts->backlight_zero)
    {
        printk("[ft5726] %s: brightness level changed only, disable touch gesture mode\n", __func__);
        ft5726_touch_power_enable();
        msleep(200);
        fts_ts_resume(&private_ts->client->dev);
    }
    else
        printk("[ft5726] %s: touch suspend check ok\n", __func__);
    return;
}

void ft5726_suspend_with_backlight(void)
{
    // add by leo for skip COS/POS ++
    if(entry_mode == 4)
    {
        printk("[ft5726] %s:[%d]: In COS, Skip\n", __func__, __LINE__);
        return;
    }
    else if(entry_mode == 3)
    {
        printk("[ft5726] %s:[%d]: In POS, Skip\n", __func__, __LINE__);
        return;
    }
    // add by leo for skip COS/POS --

    spin_lock(&private_ts->backlight_lock);
    //printk("[ft5726] %s: start \n", __func__);
    if(!private_ts->backlight_zero)
        private_ts->backlight_zero = true;
    spin_unlock(&private_ts->backlight_lock);

    fts_ts_suspend(&private_ts->client->dev);
    queue_delayed_work(private_ts->ts_update_workqueue, &private_ts->touch_suspend_check_work, 1 * HZ);
    return;
}
EXPORT_SYMBOL(ft5726_suspend_with_backlight);

void ft5726_resume_with_backlight(void)
{
    // add by leo for skip COS/POS ++
    if(entry_mode == 4)
    {
    ...
    //printk("[ft5726] %s: start \n", __func__);
    if(private_ts->backlight_zero)
        private_ts->backlight_zero = false;
    spin_unlock(&private_ts->backlight_lock);
...
EXPORT_SYMBOL(ft5726_resume_with_backlight);
...
void ft5726_touch_power_enable(void)
{
    ...
    ft5726_irq_disable();
    /*
    err = gpio_direction_output(private_ts->pdata->power_gpio, 0);
    if(err)
    	printk("[ft5726] %s: Failed to set power_gpio direction low, error=%d\n", __func__, err);

    mdelay(2);
    */
    err = gpio_direction_output(private_ts->pdata->reset_gpio, 0);
    if(err)
        printk("[ft5726] %s: Failed to set reset_gpio direction low, error=%d\n", __func__, err);
    gpio_set_value(private_ts->pdata->reset_gpio, 0);

    udelay(200);

    err = gpio_direction_output(private_ts->pdata->power_gpio, 1);
    if(err)
        printk("[ft5726] %s: Failed to set power_gpio direction high, error=%d\n", __func__, err);
    ...
    //msleep(private_ts->pdata->soft_rst_dly);
    return;
}
EXPORT_SYMBOL(ft5726_touch_power_enable);
...
static void ft5726_send_ac_cmd(struct work_struct *work)
{
    ...
    {
        case TYPE_NONE:
            printk("[ft5726] %s: TYPE_NONE\n", __func__);
            ret = fts_write_reg(private_ts->client, FTS_USB_PLUG_IN, usb_out);
            if (ret < 0)
                printk("[ft5726] %s: write FTS_USB_PLUG_IN fail, ret =%d \n", __func__, ret);
            break;

        case AC_IN:
            printk("[ft5726] %s: AC_IN\n", __func__);
        ...
        default:
            printk("[ft5726] %s: wrong cable type ..\n", __func__);
            break;
    }

    ret = fts_read_reg(private_ts->client, FTS_USB_PLUG_IN, &state);
    if(ret < 0)
        printk("[ft5726] %s: read FTS_USB_PLUG_IN fail, ret =%d \n", __func__, ret);
...
int ft5726_cable_status_handler(int state)
{
    ...
    if(entry_mode == 4)
    {
        printk("[ft5726] %s:[%d]: In COS, Skip usb notifier\n", __func__, __LINE__);
        return 0;
    }
    else if(entry_mode == 3)
    {
        printk("[ft5726] %s:[%d]: In POS, Skip usb notifier\n", __func__, __LINE__);
        return 0;
    }
    // add by leo for skip COS/POS --

    printk("[ft5726] %s: cable state = %d\n", __func__, state);
    usb_state = state;
    if(private_ts->suspended)
    {
        printk("[ft5726] %s: Skip cable status notifier when system suspend\n", __func__);
        return 0;
    }
...
static ssize_t ft5726_proc_gesture_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    ...
    printk("[ft5726] %s: ts->gesture = 0x%02x \n", __func__, private_ts->gesture);
    return 0;
}

static ssize_t ft5726_proc_gesture_write(struct file *filp, const char __user *buf, size_t count, loff_t *off)
{
    printk("[ft5726] %s: GESTURE_DOUBLE_CLICK = %c, GESTURE_ENABLE = %c, GESTURE_W = %c, GESTURE_S = %c, GESTURE_E = %c, GESTURE_C = %c, GESTURE_Z = %c, GESTURE_V = %c \n",
           __func__, buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7]);

    if((buf[0] == '0') && (buf[1] == '0'))
    {
        private_ts->gesture = (private_ts->gesture & (~_GESTURE_ENABLE));
        printk("[ft5726] %s: Gesture Disable (private_ts->gesture = 0x%02x)\n", __func__, private_ts->gesture);
    }
    else
    {
        ...
        private_ts->gesture = (private_ts->gesture | _GESTURE_ENABLE);
        printk("[ft5726] %s: Gesture Enable (private_ts->gesture = 0x%02x) \n", __func__, private_ts->gesture);
    }
...
{
    printk(KERN_ERR "[ft5726] %s: START\n", __func__);

    ft5726_debug = !ft5726_debug;
    msleep(30);
    printk(KERN_ERR "[ft5726] %s: ft5726_debug = %d\n", __func__, ft5726_debug);
    return 0;
}
...
static ssize_t ft5726_proc_tp_debug_write(struct file *filp, const char __user *buf, size_t count, loff_t *off)
{
    ...
    {
        printk("[ft5726] %s: get intr_pin = %d\n", __func__, gpio_get_value(private_ts->pdata->irq_gpio));

        if(buf[8] == '0')
        {
            gpio_direction_output(private_ts->pdata->irq_gpio, 0);
            printk("[ft5726] %s: set intr_pin. = %s\n", __func__, "Low");
        }
        else if(buf[8] == '1')
        {
    ...
    else if((buf[0] == 'p') && (buf[1] == 'o') && (buf[2] == 'w') && (buf[3] == 'e') && (buf[4] == 'r') && (buf[5] == 'p') && (buf[6] == 'i') && (buf[7] == 'n'))
    {
        if(buf[9] == '0')
        {
            ...
            printk("[ft5726] %s: set power_pin. = %s\n", __func__, "Low");
        }
        else if(buf[9] == '1')
        {
    ...
    else if((buf[0] == 'r') && (buf[1] == 'e') && (buf[2] == 'c') && (buf[3] == 'v'))
    {
        ...
        if(err < 0)
            printk("[ft5726] Received the packet Error.\n");
    }
    else if((buf[0] == 'r') && (buf[1] == 'e') && (buf[2] == 'l') && (buf[3] == 'o') && (buf[4] == 'a') && (buf[5] == 'd'))
    {
        //if(buf[7] == '1')
        //{
        printk("[ft5726] %s: reload... %s\n", __func__, buf);
    ...
    if((buf[0] == 's') && (buf[1] == 'u') && (buf[2] == 's') && (buf[3] == 'p') && (buf[4] == 'e') && (buf[5] == 'n') && (buf[6] == 'd'))
    {
        if(buf[8] == '0')
        {
            printk("[ft5726] %s: fts_ts_suspend ... \n", __func__);
            fts_ts_suspend(&private_ts->client->dev);
        }
        else if(buf[8] == '1')
        {
            printk("[ft5726] %s: fts_ts_resume ... \n", __func__);
            fts_ts_resume(&private_ts->client->dev);
        }
    }
    // JUST FOR TEST
    ...
    {
        printk("[ft5726] %s: debug_function = testtest \n", __func__);
        ...
        {
            printk("[ft5726] %s: report gesture KEY_POWER2 test. \n", __func__);
        ...
        {
            printk("[ft5726] %s: report gesture Double Click test. \n", __func__);
        ...
        {
            printk("[ft5726] %s: report gesture W test. \n", __func__);
        ...
        {
            printk("[ft5726] %s: report gesture S test. \n", __func__);
        ...
        {
            printk("[ft5726] %s: report gesture e test.", __func__);
        ...
        {
            printk("[ft5726] %s: report gesture C test. \n", __func__);
        ...
        {
            printk("[ft5726] %s: report gesture V test. \n", __func__);
...
static void ft5726_create_proc_tp_debug_file(void)
{
    ...
    if(ft5726_proc_tp_debug_file)
        printk("[ft5726] %s:[%d]: proc TP debug file create sucessed\n", __func__, __LINE__);
    else
        printk("[ft5726] %s:[%d]: proc TP debug file create failed\n", __func__, __LINE__);
    return;
}
...
{
    extern struct proc_dir_entry proc_root;
    printk("[ft5726] %s:[%d]: proc debug file removed.\n", __func__, __LINE__);
...
static void ft5726_create_proc_tp_test_file(void)
{
    ...
    if(ft5726_proc_tp_test_file)
        printk("[ft5726] %s:[%d]: proc TP test file create sucessed\n", __func__, __LINE__);
    else
        printk("[ft5726] %s:[%d]: proc TP test file create failed\n", __func__, __LINE__);
    return;
}
...
{
    return sprintf(buf, "%d-%d-%X-%X.%d.%d\n", FT5726_LCM_ID, FT5726_TP_ID, private_ts->fw_vendor_id, private_ts->fw_ver[0], private_ts->fw_ver[1], private_ts->fw_ver[2]);
}
...
static void fts_fw_update_work(struct work_struct *work)
{
    ...
    if(FT5726_PROJ_ID != 2) // ZT581KL
    {
        printk("[ft5726] %s: Project ZT581KL touch FW update.\n", __func__);
        if((FT5726_HW_ID != 5)&&(private_ts->fw_ver[0] < TOUCH_04MM_TP_DEFINE))// not MP board and 0.55mm TP
        {
            printk("[ft5726] %s: old 0.55mm TP FW update.\n", __func__);
            if(FT5726_TP_ID)
                fts_ctpm_fw_upgrade_with_app_file(fts_wq_data->client, TOUCH_055MM_FW_BIEL_PATH);
            else
                fts_ctpm_fw_upgrade_with_app_file(fts_wq_data->client, TOUCH_055MM_FW_GIS_PATH);
        }
        else // MP board or 0.4mm TP
        {
            printk("[ft5726] %s: new 0.4mm TP FW update.\n", __func__);
            if(FT5726_TP_ID) // BIEL
            {
                if(FT5726_LCM_ID) // TIANMA
                    fts_ctpm_fw_upgrade_with_app_file(fts_wq_data->client, TOUCH_FW_TIANMA_BIEL_PATH);
                else // SHARP
                    fts_ctpm_fw_upgrade_with_app_file(fts_wq_data->client, TOUCH_FW_SHARP_BIEL_PATH);
            }
            else // GIS
            {
                if(FT5726_LCM_ID) // TIANMA
                    fts_ctpm_fw_upgrade_with_app_file(fts_wq_data->client, TOUCH_FW_TIANMA_GIS_PATH);
                else // SHARP
                    fts_ctpm_fw_upgrade_with_app_file(fts_wq_data->client, TOUCH_FW_SHARP_GIS_PATH);
            }
        }
    }
    else //Z581KL
    {
        printk("[ft5726] %s: Project Z581KL touch FW update.\n", __func__);
        printk("[ft5726] %s: new 0.4mm TP FW update.\n", __func__);
        if(FT5726_TP_ID) // BIEL
        {
            if(FT5726_LCM_ID) // TIANMA
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk("[ft5726] %s: START v18 \n", __func__); // add by leo for testtest

    // add by leo for skip COS/POS ++
    if(entry_mode == 4)
    {
        printk("[ft5726] %s:[%d]: In COS, Skip Probe\n", __func__, __LINE__);
        return 0;
    }
    else if(entry_mode == 3)
    {
        printk("[ft5726] %s:[%d]: In POS, Skip Probe\n", __func__, __LINE__);
        return 0;
    }
    // add by leo for skip COS/POS --
    ...
    printk("[ft5726] %s: FT5726_HW_ID = %d \n", __func__, FT5726_HW_ID);

    FT5726_PROJ_ID = Read_PROJ_ID();
    printk("[ft5726] %s: FT5726_PROJ_ID = %d \n", __func__, FT5726_PROJ_ID);

    printk("[ft5726] %s: set reset_gpio direction low ... \n", __func__);
    err = gpio_request(pdata->reset_gpio, "focaltech-reset");
    if(err < 0)
        printk(KERN_ERR "[ft5726] %s: Failed to request GPIO %d (focaltech-reset) error=%d\n", __func__, pdata->reset_gpio, err);
    ...
    printk("[ft5726] %s: set VDD direction high ... \n", __func__);
    err = gpio_request(pdata->power_gpio, "focaltech-power");
    if(err < 0)
        printk(KERN_ERR "[ft5726] %s: Failed to request GPIO %d (focaltech-power) error=%d\n", __func__, pdata->power_gpio, err);
    ...
    //printk("[ft5726] %s: check TP_ID ... \n", __func__);
    err = gpio_request(pdata->tpid_gpio, "focaltech-tpid");
    if(err < 0)
        printk(KERN_ERR "[ft5726] %s: Failed to request GPIO %d (focaltech-tpid) error=%d\n", __func__, pdata->tpid_gpio, err);

    FT5726_TP_ID = gpio_get_value(pdata->tpid_gpio);
    printk("[ft5726] %s: FT5726_TP_ID = %d \n", __func__, FT5726_TP_ID);

    FT5726_LCM_ID = gpio_get_value(pdata->lcmid_gpio);
    printk("[ft5726] %s: FT5726_LCM_ID = %d \n", __func__, FT5726_LCM_ID);
    // add by leo --
    ...
    wake_lock_init(&data->wakelock, WAKE_LOCK_SUSPEND, "focal-touchscreen");
    private_ts = data; // add by leo for testtest

    input_dev->name = "focal-touchscreen";
    ...
    spin_lock_init(&data->backlight_lock);
    ...
    printk("[ft5726] %s: configure touch gpio & power on by reset ... \n", __func__); // add by leo for testtest
    ...
    printk("[ft5726] %s: create sysfs nodes ... \n", __func__); // add by leo for testtest
    ...
    printk("[ft5726] %s: request touch interrupt, client->irq = %d, gpio_to_irq(pdata->irq_gpio) = %d ... \n",
           __func__,client->irq, gpio_to_irq(pdata->irq_gpio)); // add by leo for testtest
    ...
    enable_irq_wake(client->irq); // add by leo for gesture wake-up
    ...
    printk("[ft5726] %s: create debug nodes ... \n", __func__); // add by leo for testtest
    ...
    printk("[ft5726] %s: read FW version ... \n", __func__); // add by leo for testtest

    fts_update_fw_ver(data);
    fts_update_fw_vendor_id(data);

    // add by leo for auto touch FW update ++
    printk("[ft5726] %s: fw_vendor_id = 0x%02X \n", __func__, data->fw_vendor_id);
    printk("[ft5726] %s: FW version check and auto-update (data->fw_ver[0] = 0x%02X) ... \n", __func__, data->fw_ver[0]);

    if(build_version == 1)
        printk("[ft5726] %s: Skip touch FW update in eng mode ...\n", __func__);
    else
    {
        if(FT5726_PROJ_ID != 2) // ZT581KL
        {
            printk("[ft5726] %s: Project ZT581KL touch FW check.\n", __func__);
            if((FT5726_HW_ID != 5)&&(data->fw_ver[0] < TOUCH_04MM_TP_DEFINE))// not MP board and 0.55mm TP
            {
                printk("[ft5726] %s: old 0.55mm TP FW version check.\n", __func__);
                if(FT5726_TP_ID) // BIEL
                {
                    if((data->fw_ver[0] < 0x21)||(data->fw_vendor_id != 0x3B))
                    {
                        queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
                        printk("[ft5726] %s: force BIEL FW update to version: 0x21\n", __func__);
                    }
                    else
                        printk("[ft5726] %s: check version pass, skip force FW update\n", __func__);
                }
                else // GIS
                {
                    if((data->fw_ver[0] < 0x20)||(data->fw_vendor_id != 0xCB))
                    {
                        queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
                        printk("[ft5726] %s: force GIS FW update to version: 0x20\n", __func__);
                    }
                    else
                        printk("[ft5726] %s: check version pass, skip force FW update\n", __func__);
                }
            ...
            {
                printk("[ft5726] %s: new 0.4mm TP FW version check.\n", __func__);
                if(FT5726_TP_ID) // BIEL
                {
                    if(FT5726_LCM_ID) // TIANMA
                    {
                        if((data->fw_ver[0] < 0x9B)||(data->fw_vendor_id != 0x3B))
                        {
                            queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
                            printk("[ft5726] %s: force TIANMA BIEL FW update to version: 0x3B_0x9B\n", __func__);
                        }
                        else
                            printk("[ft5726] %s: check TIANMA BIEL version pass, skip force FW update\n", __func__);
                    }
                    else // SHARP
                    {
                        if((data->fw_ver[0] < 0x99)||(data->fw_vendor_id != 0x3A))
                        {
                            queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
                            printk("[ft5726] %s: force SHARP BIEL FW update to version: 0x3A_0x99\n", __func__);
                        }
                        else
                            printk("[ft5726] %s: check SHARP BIEL version pass, skip force FW update\n", __func__);
                    }
                }
                else // GIS
                {
                    if(FT5726_LCM_ID) // TIANMA
                    {
                        if((data->fw_ver[0] < 0x92)||(data->fw_vendor_id != 0xCB))
                        {
                            queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
                            printk("[ft5726] %s: force TIANMA GIS FW update to version: 0xCB_0x92\n", __func__);
                        }
                        else
                            printk("[ft5726] %s: check TIANMA GIS version pass, skip force FW update\n", __func__);
                    }
                    else // SHARP
                    {
                        if((data->fw_ver[0] < 0x90)||(data->fw_vendor_id != 0xCA))
                        {
                            queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
                            printk("[ft5726] %s: force SHARP GIS FW update to version: 0xCA_0x90\n", __func__);
                        }
                        else
                            printk("[ft5726] %s: check SHARP GIS version pass, skip force FW update\n", __func__);
                    }
                }
            }
        }
        ...
        {
            printk("[ft5726] %s: Project Z581KL touch FW check.\n", __func__);
            printk("[ft5726] %s: new 0.4mm TP FW version check.\n", __func__);
            if(FT5726_TP_ID) // BIEL
            {
                if(FT5726_LCM_ID) // TIANMA
...
//add by leo ++
    ...
    {
        printk("[ft5726] %s: switch_dev_register failed!\n", __func__);
    }
//add by leo --
...
{
    printk("[ft5726] %s: touch driver initialize \n", __func__); // add by leo for testtest
