...
#include "asus_typec_core_ops.c"
#endif

static void rt1711_init_gpio(struct device *dev)
{
	struct device_node *np = dev->of_node;
	int gpio = of_get_named_gpio(np, "asus,otg_en", 0);
	int ret;

	pr_err("%s: got gpio of otg_en: %d\n", __func__, gpio);
	ret = devm_gpio_request_one(dev, gpio, GPIOF_OUT_INIT_LOW, "PMI8952_OTG_EN");
	if (ret < 0) {
		pr_err("%s: otg_en request error, ret=%d\n", __func__, ret);
	}
}
...
#if defined(CONFIG_TYPEC)
	ret = rt1711_typec_ops_init(chip);
	if (ret < 0)
		return ret;

	ret = add_typec_device(chip->dev, &rt1711_typec_ops);
	if (ret < 0) {
		pr_err("%s: add_typec_device fail, err = %d\n", __func__, ret);
		return ret;
	}
#endif
	pr_info("%s probe OK!\n", __func__);
