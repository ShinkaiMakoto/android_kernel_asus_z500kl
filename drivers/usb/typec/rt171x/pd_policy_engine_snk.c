...
static inline uint32_t pd_pdo_object_position(uint32_t rdo)
{
	uint32_t pos = ((rdo) >> 28) & 0x07;
	return (pos > 0) ?  --pos : 0;
}
...
#if defined(CONFIG_TYPEC)
	/* According to Asus/power team rules, we need to do some charger
	 * setting before sending the request. */
	index = pd_pdo_object_position(pd_port->last_rdo);
	if (index >= 0 && index < src_cap->nr) {
		pdo = src_cap->pdos[index];
		tcpci_sink_vbus_prerequisite(pd_port->tcpc_dev, pdo);
	}
#endif
