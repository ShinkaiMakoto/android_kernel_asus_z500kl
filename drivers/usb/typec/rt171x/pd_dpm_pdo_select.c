...
/*
  ...
  * This function is rewritten by Asus. The selection algorithm is sit on top
  * of 'MAX_POWER' with a little modification.
  * 1. It will override the requirement of sink, let maximum operating current
  *    equals to the source cap. Because some rejection is due to maximum
  *    current mismatch.
  */
...
static bool dpm_select_pdo_from_custom(
	...
	struct dpm_pdo_info_t *sink, struct dpm_pdo_info_t *source)
{
	if (!dpm_select_pdo_from_max_power(select_info, sink, source))
		return false;

	/* succeeded in dpm_select_pdo_from_max_power */
	if (sink->type == DPM_PDO_TYPE_BAT)
		sink->uw = source->uw;
	else
		sink->ma = source->ma;
