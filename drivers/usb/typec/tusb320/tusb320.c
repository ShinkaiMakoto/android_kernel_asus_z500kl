...
static bool tusb320_i2c_status(void);
static int tusb320_cc_status(void);
...
struct typec_device_ops tusb320_ops = {
	...
	.i2c_status = tusb320_i2c_status,
	.cc_status = tusb320_cc_status,
};
...
static int tusb320_cc_dir(void)
{
	u8 reg_val;
	int ret;
	enum typec_attached_state attached_state = TYPEC_NOT_ATTACHED;
...
static void tusb320_intb_work(struct work_struct *work)
{
	...
	if ( attached_state != TYPEC_NOT_ATTACHED){
		di->cc_valid = tusb320_cc_dir();
	}
	mutex_unlock(&di->mutex);
			 ...
			 const struct i2c_device_id *id)
{
	...
	if(!of_property_read_u8(node, "tusb320_typec,gpio_enb-pins", &num_enb_pins)) {
		hardware_phase = Read_HW_ID_STR();
		pr_info("%s: hardware phase: %s\n", __func__, hardware_phase);
		gpio_idx = of_property_match_string(node, "tusb320_typec,gpio_enb-names", hardware_phase);
		if( gpio_idx < 0)
			gpio_idx = 0;
	};
   
	di->gpio_enb = of_get_named_gpio(node, "tusb320_typec,gpio_enb", gpio_idx);
	if (!gpio_is_valid(di->gpio_enb)) {
		pr_err("%s: of get gpio_enb error\n", __func__);
		ret = -EINVAL;
		goto err_i2c_check_functionality_0;
	}
	pr_info("%s: enable gpio = %d\n", __func__, di->gpio_enb);
	ret = gpio_request_one(di->gpio_enb, GPIOF_OUT_INIT_LOW, "tusb320_en");
	if (ret < 0) {
	...
	/* We have no idea the initial value of enable pin.
	 * Maximum value of T_I2C_EN is 100ms */
	timeout = jiffies + msecs_to_jiffies(110);
	do {
	...
	ret = gpio_request_one(di->gpio_intb, GPIOF_IN, "tusb320_int");
	if (ret < 0) {
