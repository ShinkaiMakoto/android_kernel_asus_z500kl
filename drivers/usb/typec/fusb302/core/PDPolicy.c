...
FSC_S32                         previous_batt_capacity;
/////////////////////////////////////////////////////////////////////////////
...
{    
...
#ifdef FSC_HAVE_SNK
    previous_batt_capacity = -1;
    SinkRequestMaxVoltage = 180;                                               // Maximum voltage that the sink will request: 180*50mA = 9V
    SinkRequestMaxPower = 36000;                                               // Maximum power the sink will request (36000*0.5mW = 18W, used to calculate current as well)
    SinkRequestOpPower = 36000;                                                // Operating power the sink will request (36000*0.5mW = 18W, sed to calculate current as well)
    ...
    SinkUSBCommCapable = TRUE;                                                 // Whether the sink is USB communications capable

    CapsHeaderSink.NumDataObjects = 3;                                          // Set the number of power objects to 3
    ...
    CapsSink[0].FPDOSink.Voltage = 100;                                         // Set 5V (unit: 50mV) for the first supply option. refer to "Table-6.10 Fixed Supply PDO - Sink" of PD specification.
    CapsSink[0].FPDOSink.OperationalCurrent = 50;                               // Set that our device will consume 500mA for this object
    CapsSink[0].FPDOSink.DataRoleSwap = 0;                                      // By default, enable DR_SWAP
    CapsSink[0].FPDOSink.USBCommCapable = 1;                                    // By default, USB communications is not allowed
    CapsSink[0].FPDOSink.ExternallyPowered = 0;                                 // By default, we are externally powered
    CapsSink[0].FPDOSink.HigherCapability = TRUE;                               // By default, require more than vSafe5V
    CapsSink[0].FPDOSink.DualRolePower = 0;                                     // By default, enable PR_SWAP
    CapsSink[0].FPDOSink.Reserved = 0;

    CapsSink[1].FPDOSink.Voltage = 100;                                         // Set 5V (unit: 50mV) for the first supply option. refer to "Table-6.10 Fixed Supply PDO - Sink" of PD specification.
    CapsSink[1].FPDOSink.OperationalCurrent = 300;                              // Set that our device will consume 3000mA for this object, unit in 10mA
    CapsSink[1].FPDOSink.DataRoleSwap = 0;                                      // Only valid for PDO[0], so set to 0
    CapsSink[1].FPDOSink.USBCommCapable = 0;                                    // Only valid for PDO[0], so set to 0 - USB communications is not allowed
    CapsSink[1].FPDOSink.ExternallyPowered = 0;                                 // Only valid for PDO[0], so set to 0
    CapsSink[1].FPDOSink.HigherCapability = 0;                                  // Only valid for PDO[0], so set to 0
    CapsSink[1].FPDOSink.DualRolePower = 0;                                     // Only valid for PDO[0], so set to 0
    CapsSink[1].FPDOSink.Reserved = 0;

    CapsSink[2].FPDOSink.Voltage = 180;                                         // Set 9V (unit: 50mV) for the second supply option
    CapsSink[2].FPDOSink.OperationalCurrent = 200;                              // Set that our device will consume 2000mA for this object, unit in 10mA
    CapsSink[2].FPDOSink.DataRoleSwap = 0;                                      // Not used
    CapsSink[2].FPDOSink.USBCommCapable = 0;                                    // Not used
    CapsSink[2].FPDOSink.ExternallyPowered = 0;                                 // Not used
    CapsSink[2].FPDOSink.HigherCapability = 0;                                  // Not used
    CapsSink[2].FPDOSink.DualRolePower = 0;                                     // Not used
#endif // FSC_HAVE_SNK
    ...
    CapsSource[0].FPDOSupply.Voltage = 100;                                     // Set 5V for the first supply option, unit in 50mV, refer to "Table-6.6 Fixed Supply PDO - Source" of PD specification.
    CapsSource[0].FPDOSupply.MaxCurrent = 45;                                  // Set 450mA for the first supply option, unit in 10mA
    ...
    CapsSource[0].FPDOSupply.USBCommCapable = TRUE;                             // By default, USB communications is not allowed
    CapsSource[0].FPDOSupply.ExternallyPowered = FALSE;                          // By default, state that we are externally powered
    CapsSource[0].FPDOSupply.USBSuspendSupport = FALSE;                         // By default, allow  USB Suspend
    CapsSource[0].FPDOSupply.DualRolePower = FALSE;                              // By default, enable PR_SWAP
    ...
    CapsSource[1].FPDOSupply.MaxCurrent = 150;                                  // Set 1.5A for the second supply option
...
#if 0
const char* pd_state_str(PolicyState_t s)
{
	const char *str[]		      = {
		[FIRST_PE_ST]		      =	"peDisabled",
		[peErrorRecovery]	      =	"peErrorRecovery",
		[peSourceHardReset]	      =	"peSourceHardReset",
		[peSourceSendHardReset]	      =	"peSourceSendHardReset",
		[peSourceSoftReset]	      =	"peSourceSoftReset",
		[peSourceSendSoftReset]	      =	"peSourceSendSoftReset",
		[peSourceStartup]	      =	"peSourceStartup",
		[peSourceSendCaps]	      =	"peSourceSendCaps",
		[peSourceDiscovery]	      =	"peSourceDiscovery",
		[peSourceDisabled]	      =	"peSourceDisabled",
		[peSourceTransitionDefault]   =	"peSourceTransitionDefault",
		[peSourceNegotiateCap]	      =	"peSourceNegotiateCap",
		[peSourceCapabilityResponse] =	"peSourceCapabilityResponse",
		[peSourceTransitionSupply]    =	"peSourceTransitionSupply",
		[peSourceReady]		      =	"peSourceReady",
		[peSourceGiveSourceCaps]      =	"peSourceGiveSourceCaps",
		[peSourceGetSinkCaps]	      =	"peSourceGetSinkCaps",
		[peSourceSendPing]	      =	"peSourceSendPing",
		[peSourceGotoMin]	      =	"peSourceGotoMin",
		[peSourceGiveSinkCaps]	      =	"peSourceGiveSinkCaps",
		[peSourceGetSourceCaps]	      =	"peSourceGetSourceCaps",
		[peSourceSendDRSwap]	      =	"peSourceSendDRSwap",
		[peSourceEvaluateDRSwap]      =	"peSourceEvaluateDRSwap",
		[peSinkHardReset]	      =	"peSinkHardReset",
		[peSinkSendHardReset]	      =	"peSinkSendHardReset",
		[peSinkSoftReset]	      =	"peSinkSoftReset",
		[peSinkSendSoftReset]	      =	"peSinkSendSoftReset",
		[peSinkTransitionDefault]     =	"peSinkTransitionDefault",
		[peSinkStartup]		      =	"peSinkStartup",
		[peSinkDiscovery]	      =	"peSinkDiscovery",
		[peSinkWaitCaps]	      =	"peSinkWaitCaps",
		[peSinkEvaluateCaps]	      =	"peSinkEvaluateCaps",
		[peSinkSelectCapability]      =	"peSinkSelectCapability",
		[peSinkTransitionSink]	      =	"peSinkTransitionSink",
		[peSinkReady]		      =	"peSinkReady",
		[peSinkGiveSinkCap]	      =	"peSinkGiveSinkCap",
		[peSinkGetSourceCap]	      =	"peSinkGetSourceCap",
		[peSinkGetSinkCap]	      =	"peSinkGetSinkCap",
		[peSinkGiveSourceCap]	      =	"peSinkGiveSourceCap",
		[peSinkSendDRSwap]	      =	"peSinkSendDRSwap",
		[peSinkEvaluateDRSwap]	      =	"peSinkEvaluateDRSwap",
		[peSourceSendVCONNSwap]	      =	"peSourceSendVCONNSwap",
		[peSinkEvaluateVCONNSwap]     =	"peSinkEvaluateVCONNSwap",
		[peSourceSendPRSwap]	      =	"peSourceSendPRSwap",
		[peSourceEvaluatePRSwap]      =	"peSourceEvaluatePRSwap",
		[peSinkSendPRSwap]	      =	"peSinkSendPRSwap",
		[peSinkEvaluatePRSwap]	      =	"peSinkEvaluatePRSwap",
		[peGiveVdm]		      =	"peGiveVdm",
		[peUfpVdmGetIdentity]	      =	"peUfpVdmGetIdentity",
		[peUfpVdmSendIdentity]	      =	"peUfpVdmSendIdentity",
		[peUfpVdmGetSvids]	      =	"peUfpVdmGetSvids",
		[peUfpVdmSendSvids]	      =	"peUfpVdmSendSvids",
		[peUfpVdmGetModes]	      =	"peUfpVdmGetModes",
		[peUfpVdmSendModes]	      =	"peUfpVdmSendModes",
		[peUfpVdmEvaluateModeEntry]   =	"peUfpVdmEvaluateModeEntry",
		[peUfpVdmModeEntryNak]	      =	"peUfpVdmModeEntryNak",
		[peUfpVdmModeEntryAck]	      =	"peUfpVdmModeEntryAck",
		[peUfpVdmModeExit]	      =	"peUfpVdmModeExit",
		[peUfpVdmModeExitNak]	      =	"peUfpVdmModeExitNak",
		[peUfpVdmModeExitAck]	      =	"peUfpVdmModeExitAck",
		[peUfpVdmAttentionRequest]    =	"peUfpVdmAttentionRequest",
		[peDfpUfpVdmIdentityRequest]  =	"peDfpUfpVdmIdentityRequest",
		[peDfpUfpVdmIdentityAcked]    =	"peDfpUfpVdmIdentityAcked",
		[peDfpUfpVdmIdentityNaked]    =	"peDfpUfpVdmIdentityNaked",
		[peDfpCblVdmIdentityRequest]  =	"peDfpCblVdmIdentityRequest",
		[peDfpCblVdmIdentityAcked]    =	"peDfpCblVdmIdentityAcked",
		[peDfpCblVdmIdentityNaked]    =	"peDfpCblVdmIdentityNaked",
		[peDfpVdmSvidsRequest]	      =	"peDfpVdmSvidsRequest",
		[peDfpVdmSvidsAcked]	      =	"peDfpVdmSvidsAcked",
		[peDfpVdmSvidsNaked]	      =	"peDfpVdmSvidsNaked",
		[peDfpVdmModesRequest]	      =	"peDfpVdmModesRequest",
		[peDfpVdmModesAcked]	      =	"peDfpVdmModesAcked",
		[peDfpVdmModesNaked]	      =	"peDfpVdmModesNaked",
		[peDfpVdmModeEntryRequest]    =	"peDfpVdmModeEntryRequest",
		[peDfpVdmModeEntryAcked]      =	"peDfpVdmModeEntryAcked",
		[peDfpVdmModeEntryNaked]      =	"peDfpVdmModeEntryNaked",
		[peDfpVdmModeExitRequest]     =	"peDfpVdmModeExitRequest",
		[peDfpVdmExitModeAcked]	      =	"peDfpVdmExitModeAcked",
		[peSrcVdmIdentityRequest]     =	"peSrcVdmIdentityRequest",
		[peSrcVdmIdentityAcked]	      =	"peSrcVdmIdentityAcked",
		[peSrcVdmIdentityNaked]	      =	"peSrcVdmIdentityNaked",
		[peDfpVdmAttentionRequest]    =	"peDfpVdmAttentionRequest",
		[peCblReady]		      =	"peCblReady",
		[peCblGetIdentity]	      =	"peCblGetIdentity",
		[peCblGetIdentityNak]	      =	"peCblGetIdentityNak",
		[peCblSendIdentity]	      =	"peCblSendIdentity",
		[peCblGetSvids]		      =	"peCblGetSvids",
		[peCblGetSvidsNak]	      =	"peCblGetSvidsNak",
		[peCblSendSvids]	      =	"peCblSendSvids",
		[peCblGetModes]		      =	"peCblGetModes",
		[peCblGetModesNak]	      =	"peCblGetModesNak",
		[peCblSendModes]	      =	"peCblSendModes",
		[peCblEvaluateModeEntry]      =	"peCblEvaluateModeEntry",
		[peCblModeEntryAck]	      =	"peCblModeEntryAck",
		[peCblModeEntryNak]	      =	"peCblModeEntryNak",
		[peCblModeExit]		      =	"peCblModeExit",
		[peCblModeExitAck]	      =	"peCblModeExitAck",
		[peCblModeExitNak]	      =	"peCblModeExitNak",
		[peDpRequestStatus]	      =	"peDpRequestStatus",
		[PE_BIST_Receive_Mode]	      =	"PE_BIST_Receive_Mode",
		[PE_BIST_Frame_Received]      =	"PE_BIST_Frame_Received",
		[PE_BIST_Carrier_Mode_2]      =	"PE_BIST_Carrier_Mode_2",
		[peSourceWaitNewCapabilities] =	"peSourceWaitNewCapabilities",
		[PE_BIST_Test_Data]	      =	"PE_BIST_Test_Data",
		[peSourceEvaluateVCONNSwap]   =	"peSourceEvaluateVCONNSwap",
		[dbgGetRxPacket]	      =	"dbgGetRxPacket",
		[dbgSendTxPacket]	      =	"dbgSendTxPacket",
	};
	return str[s];
}
#endif
...
#if 0
	pr_info("FUSB %s: %d:%s -> %d:%s\n", __func__,
		LastPolicyState, pd_state_str(LastPolicyState),
		PolicyState, pd_state_str(PolicyState));
#endif
    }
#endif // FSC_DEBUG
...
void PolicySinkWaitCaps(void)
{
    ...
    {
	    pr_err("%s: no response, error recovery\n", __func__);
        PolicyState = peErrorRecovery;
        PolicySubIndex = 0;
    }
    else if ((PolicyStateTimer == 0) && (HardResetCounter <= nHardResetCount))
    {
	    pr_err("%s: timeout, hardreset\n", __func__);
...
void PolicySinkEvaluateCaps(void)
{
    ...
    FSC_U32 objCurrent, objPower, MaxPower, SelVoltage, ReqCurrent, SelCurrent;
    FSC_U32 mSinkRequestMaxVoltage, mSinkRequestMaxPower, mSinkRequestOpPower;
    FSC_S32 batt_cap = platform_get_battery_capacity();
    doDataObject_t data_object;
    objCurrent = 0;
    NoResponseTimer = T_TIMER_DISABLE;                                          // Stop the no response timer
    ...
    /* Asus rule: while battery capacity is less than 1%, charging parameter is
     * limited to 5V/1A. */
    if (batt_cap < BATT_SHUTDOWN_TH) {
	    mSinkRequestMaxVoltage = 100;                                        // Maximum voltage that the sink will request: 100*50mA = 5V
	    mSinkRequestMaxPower = 10000;                                        // Maximum power the sink will request (10000*0.5mW = 5W, used to calculate current as well)
	    mSinkRequestOpPower = 10000;                                         // Operating power the sink will request (10000*0.5W = 5W, sed to calculate current as well)
    } else {
	    mSinkRequestMaxVoltage = SinkRequestMaxVoltage;
	    mSinkRequestMaxPower = SinkRequestMaxPower;
	    mSinkRequestOpPower = SinkRequestOpPower;
    }
    ...
    for (i=0; i<CapsHeaderReceived.NumDataObjects; i++)                         // Going to select the highest power object that we are compatible with
    {
        switch (CapsReceived[i].PDO.SupplyType)
        {
            case pdoTypeFixed:
                ...
                if (objVoltage > mSinkRequestMaxVoltage)                         // If the voltage is greater than our limit...
		    ...
		    /*
		    pr_info("FUSB %s: PDO[%d] received: fixed, %u mV, %u mA\n", __func__,
			    i, objVoltage*50, objCurrent*10);
		    */
                }
                break;
            case pdoTypeVariable:
                objVoltage = CapsReceived[i].VPDO.MaxVoltage;                   // Grab the maximum voltage of the variable supply
                if (objVoltage > mSinkRequestMaxVoltage)                         // If the max voltage is greater than our limit...
                    continue;                                               // Set the power to zero to ignore the object
                else                                                            // Otherwise...
                {
		    ...
		    /*
		    pr_info("FUSB %s: PDO[%d] received: variable, %u mV, %u mA\n", __func__,
			    i, objVoltage*50, objCurrent*10);
		    */
                }
                break;
	...
	if ((objPower == MaxPower) && (objVoltage < SelVoltage)) {
		// The current object has power equal the previous objects AND voltage is less than the previous.
		MaxPower = objPower;                                            // Store the objects power
		SelVoltage = objVoltage;                                        // Store the objects voltage (used for calculations)
		SelCurrent = objCurrent;
		reqPos = i + 1;                                                 // Store the position of the object
	} else if (objPower > MaxPower)                                         // If the current object has power greater than the previous objects
        {
            MaxPower = objPower;                                                // Store the objects power
            SelVoltage = objVoltage;                                            // Store the objects voltage (used for calculations)
	    SelCurrent = objCurrent;
    ...
    if ((reqPos == 0) && (CapsHeaderReceived.NumDataObjects > 0)) {
	    /* In case - there is no valid PDO satisfy the above selection algorithm.
	     * re-evaluate the first PDO for case - maximum current is 0A, but voltage is valid.
	     * e.g: Ellisys test case: TD.PD.SNK.E11 PDO transition - Testing upstream port */
	    if (CapsReceived[0].PDO.SupplyType == pdoTypeFixed) {
		    objVoltage = CapsReceived[0].FPDOSupply.Voltage;
		    if (objVoltage <= mSinkRequestMaxVoltage) {
			    /* Voltage is in spec. */
			    objCurrent = CapsReceived[i].FPDOSupply.MaxCurrent;
			    reqPos = 1;
		    }
	    }
	    else if (CapsReceived[0].PDO.SupplyType == pdoTypeVariable) {
		    objVoltage = CapsReceived[0].VPDO.MaxVoltage;
		    if (objVoltage <= mSinkRequestMaxVoltage) {
			    /* Voltage is in spec */
...
#if 1
	mSinkRequestOpPower = min(MaxPower, mSinkRequestMaxPower) ; /* Asus. Since there is no way to dtermine proper request current at runtime til now.
								     * We set it to maximum. Thus operating current is equal to the selected
								     * data object. */
#endif
        ReqCurrent = mSinkRequestOpPower / SelVoltage;                           // Calculate the requested operating current
        SinkRequest.FVRDO.OpCurrent = (ReqCurrent & 0x3FF);                     // Set the current based on the selected voltage (in 10mA units)
        ReqCurrent = mSinkRequestMaxPower / SelVoltage;                          // Calculate the requested maximum current
        ...
        {
            if (SelCurrent < ReqCurrent)                                        // If the max power available is less than the max power requested...
            {
                SinkRequest.FVRDO.CapabilityMismatch = TRUE;                    // flag the source that we need more power
                SinkRequest.FVRDO.MinMaxCurrent = SelCurrent;                     // Set operating power to max available
                SinkRequest.FVRDO.OpCurrent = SelCurrent;
            }
            else                                                                // Otherwise...
            {
        ...
        PolicySubIndex = 0;                                                     // Reset the sub index
        PolicyStateTimer = 2 * tSenderResponse; // Initialize the sender response timer to 2*tSenderResponse, this allow
						// retransmission in sending request.

	/* Asus rule: while battery capacity is less than 1%, charging parameter should be limited in 5V/1A.
	 * We will report operating power and maximum power as 5W in request.
	 * Here we override received data object that will be used to notify
	 * charger subsystem.
	 * So, the parameters between charger and PD source are aligned. */
	if (batt_cap < BATT_SHUTDOWN_TH) {
		data_object = CapsReceived[SinkRequest.FVRDO.ObjectPosition - 1];
		switch (data_object.PDO.SupplyType) {
		case pdoTypeFixed:
		case pdoTypeVariable:
			if (data_object.FPDOSupply.MaxCurrent > 100)
				data_object.FPDOSupply.MaxCurrent = 100;
			break;
		default:;
		}
		CapsReceived[SinkRequest.FVRDO.ObjectPosition - 1] = data_object;
	}
	/* Asus rule. Before we are going to send request, notify charger to stop charging. */
	data_object = CapsReceived[SinkRequest.FVRDO.ObjectPosition - 1];
	switch (data_object.PDO.SupplyType) {
	case pdoTypeFixed:
		data_object.FPDOSupply.MaxCurrent = 0;
		data_object.FPDOSupply.Voltage = 0;
		platform_notify_pd_sink_ready(data_object.object);
		break;
	case pdoTypeVariable:
		data_object.VPDO.MaxCurrent = 0;
		data_object.VPDO.MaxVoltage = 0;
		data_object.VPDO.MaxVoltage = 0;
		platform_notify_pd_sink_ready(data_object.object);
		break;
	default:;
	...
	pr_err("%s: no valid PDO, return to peSinkWaitCaps\n", __func__);
    }
}
    ...
    switch (PolicySubIndex)
    {
       ...
       case 1:
            if (ProtocolMsgRx)
            {
...
#ifdef FSC_DEBUG
			    pr_info("FUSB %s: PDO: 0x%08x accept by peer\n",
				    __func__, SinkRequest.object);
#endif
                            break;
                        case CMTWait:
                        case CMTReject:
			    ...
			    NoResponseTimer = tNoResponse;                      // We are going to perform software reset,
										// initialize the no response timer avoid timer experied in the
										// following state - peSinkWaitCaps.
                            break;
                        default:
                            PolicyState = peSinkSendSoftReset;                  // We are going to send a reset message for all other commands received
			    NoResponseTimer = tNoResponse;                      // Software reset received, initialize the no response timer avoid timer experied in the
										// following state - peSinkWaitCaps.
                            break;
                    }
                }
	    ...
	    else if ((PolicyStateTimer > 0) && (NoResponseTimer == 0)) {
		    /* No response, reset subindex for retransmission */
		    pr_info("%s:%d\n", __func__, __LINE__);
		    PolicySubIndex = 0;                                         // Reset the sub index
		    PDTxStatus = txIdle;                                        // Reset the transmitter status
	    }
...
void PolicySinkTransitionSink(void)
{
    ...
    if (ProtocolMsgRx)
    {
        ...
        if (PolicyRxHeader.NumDataObjects == 0)                                 // If we have received a control message...
        {
            ...
            {
                case CMTPS_RDY:
			idx = SinkRequest.FVRDO.ObjectPosition - 1;
			if (idx >= 0 && idx < sizeof(CapsReceived)/sizeof(CapsReceived[0]) &&
			    !platform_notify_pd_sink_ready(CapsReceived[idx].object))
				PolicyState = peSinkReady;                      // Go to the ready state when every thing works well.
			else
				PolicyState = peSinkSendSoftReset;              // Unable to notify platform sub-systems, e.g: charger.
										// Send a soft reset to get back to a known state.
                    break;
                case CMTSoftReset:
                    PolicyState = peSinkSoftReset;                              // Go to the soft reset state if we received a reset command
                    break;
...
#endif // FSC_HAVE_VDM
    if (CapsSink[0].FPDOSupply.DataRoleSwap == FALSE) {
...
void PolicySinkEvaluatePRSwap(void)
{
    ...
    {
        case 0: // Send either the Accept or Reject command
		if ((CapsSink[0].FPDOSupply.DualRolePower == FALSE) ||
...
/**
 * Battery capacity is cross over the defined threshold.
 * Invoke state machine to re-evaluating the source caps
 * and request the proper cap.
 */
static void do_pd_handle_batt_capacity_change(void)
{
	switch (PolicyState) {
        case peSinkEvaluateCaps:
        case peSinkSelectCapability:
        case peSinkTransitionSink:
		platform_pd_queue_delayed_work(3000); /* give it more time to finish it's work */
		return;
        case peSinkReady:
		platform_pd_queue_delayed_work(0);
		return;
	default:;
		//pr_err("invalid state: %s\n", pd_state_str(PolicyState));
	}
}

static void pd_handle_batt_capacity_change(void)
{
	FSC_S32 batt_cap = platform_get_battery_capacity();
	if (previous_batt_capacity < 0) {
		/* This is first time we receive battery capacity change event. */
		previous_batt_capacity = batt_cap;
		/* if (batt_cap < BATT_SHUTDOWN_TH) */
		/* 	do_pd_handle_batt_capacity_change(); */
		return;
	}
	if (batt_cap > previous_batt_capacity) {
		/* going up. */
		if ((batt_cap >= BATT_SHUTDOWN_TH) && (previous_batt_capacity < BATT_SHUTDOWN_TH)) {
			previous_batt_capacity = batt_cap;
			do_pd_handle_batt_capacity_change();
			return;
		}
...
void pd_handle_external_event(external_event evt)
{
	switch (evt) {
	case EXTEVT_BATT_CAPACITY_CHANGE:
		/* PD is playing as sink role. */
		pd_handle_batt_capacity_change();
		break;
	default:;
	}
}
/* return TRUE when we received source capability in time (4000ms) */
FSC_BOOL pd_wait_for_source_caps(void)
{
	FSC_U32 i;
	...
	return i >= 400 ? FALSE : TRUE;
}

FSC_BOOL pd_is_pr_swapable(void)
{
	return (CapsSink[0].FPDOSink.DualRolePower > 0 &&
		CapsSource[0].FPDOSupply.DualRolePower > 0);
}
