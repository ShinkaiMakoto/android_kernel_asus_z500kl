...
{
	FSC_U8 reset_val = 0x03;
	if (prefer_portype == USBTypeC_UNDEFINED) {
		DeviceWrite(regReset, 1, &reset_val);
    ...
    SourceCurrent = utccDefault;                                                   // Set default host current (500mA)
    updateSourceCurrent();
...
void StateMachineTypeC(void)
{
	...
	if (ConnState != previous_state) {
		pr_info("FUSB  %s ConnState changed: %s -> %s\n", __func__,
...
void StateMachineUnattached(void)
{
    if (Registers.Status.I_TOGDONE)
    {
	    if (PortType != USBTypeC_Source) {
		    /* exam VBUS status. */
		    possible_pd = Registers.Status.VBUSOK == 0;
		    pr_info("fusb %s: the peer is %spossibple a PD capable device\n",
			    __func__, possible_pd ? "" : "im");
	    }
...
void SetStateAttachWaitSource(void)
{
    ...
    platform_enable_timer(TRUE);
#endif
    pr_info("%s - loopCounter = %d \n", __func__, loopCounter);
    /* Mark Y cable (try to play as source role and VBUS present) as illegal cable */
    if(loopCounter++ > MAX_CABLE_LOOP || Registers.Status.VBUSOK)  // Swap toggle state machine current if looping
    {
#ifndef FSC_ILLEGAL_CABLE
        toggleCurrentSwap();
        loopCounter = 0;
#else
	if (!platform_in_dual_role_swap_attempts()) {
...
ConnectionState StateMachineGetConnectingState(struct connecting_state_descriptor *desc)
{
	/* TODO.
	 * dealing concurrent access to 'ConnState' */
	if (desc) {
		desc->cc1 = blnCCPinIsCC1;
		desc->cc2 = blnCCPinIsCC2;
		desc->conn_state = ConnState;
		desc->termination_type = DecodeCCTermination();
	}
	return ConnState;
}
...
void typec_handle_external_event(external_event evt)
{
	switch (evt) {
	case EXTEVT_BATT_CAPACITY_CHANGE:
		if (USBPDActive) {
			pd_handle_external_event(evt);
		}
		break;
	default:;
	}
...
FSC_BOOL typec_pd_wait_for_source_caps(void)
{
	if (possible_pd)
		return pd_wait_for_source_caps();
	return FALSE;
}

FSC_BOOL typec_pd_is_pr_swapable(void)
{
	return pd_is_pr_swapable();
}
...
void StateMachineIllegalCable(void)
{
	...
	pr_info("%s g_Unattach_Fail = TRUE \n", __func__);
	if(Registers.Status.I_COMP_CHNG == 1)
	{
		CCTermPrevious = DecodeCCTermination();
