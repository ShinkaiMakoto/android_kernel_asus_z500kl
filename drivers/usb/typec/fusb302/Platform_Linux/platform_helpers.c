...
extern const char* bus_clkname[FUSB_NUM_BUS_CLOCKS];
...
static unsigned long timestamp_i2c_error = INITIAL_JIFFIES;

/**
 * @return 0 when platform is able to perform hardware reset of chip
 */
int fusb_hardware_reset(void)
{
#if 0
	return 0;
#else
...
/* In some buggy platform, it will not be able to reboot after BUG_ON() asserted.
 * In this case, always de-assert by define REBOOTABLE to 0. */
#define REBOOTABLE 0
void fusb_vote_error_recovery(void)
{
	unsigned long timeout;
	struct timespec boottime;

	if (timestamp_i2c_error == INITIAL_JIFFIES) {
		/* 1'st error */
		timestamp_i2c_error = jiffies;
		return;
	}
	timeout = timestamp_i2c_error + 3*HZ;
	if (time_after(jiffies, timeout)) {
		/* I2C stucks for a long interval. Recovery it. */
		if (!fusb_hardware_reset()) {
			/* success in hardware reset.
			 * Clear timestamp */
			timestamp_i2c_error = INITIAL_JIFFIES;
			return;
		}
		g_I2C_Fail = TRUE;
		get_monotonic_boottime(&boottime);
		if (boottime.tv_sec < 30) {
			/* During boot, assert bug to trigger reboot. */
			BUG_ON(REBOOTABLE);
		} else {
			/* Normal runtime, just report a bug here. */
			WARN(1, "Unable to reset FUSB.\n");

			/* At the meantime, FUSB is not accessible,
			 * INT line may be pulled to low level.
			 * If we are level trigger, We need to disable
			 * IRQ to prevent CPU busy on this INT.
			 *
			 * TODO. Also need to schedule a timer to test whether
			 * FUSB comes back again. */
		}
		timestamp_i2c_error = INITIAL_JIFFIES;
	}
}

static inline void fusb_reset_error_recovery(void)
{
#if 1
	timestamp_i2c_error = INITIAL_JIFFIES;
	g_I2C_Fail = FALSE;
...
FSC_S32 fusb_InitializeGPIO(void)
{
    ...
    // VBus 5V
    if (!of_property_read_u8(node, "fairchild,vbus5v-pins", &num_pins)) {
	    hardware_phase = Read_HW_ID_STR();
	    dev_err(&chip->client->dev, "hardware phase: %s\n", hardware_phase);
	    gpio_idx = of_property_match_string(node, "fairchild,vbus5v-hw_phase", hardware_phase);
	    if( gpio_idx < 0)
		    gpio_idx = 0;
    }
    chip->gpio_VBus5V = of_get_named_gpio_flags(node, FUSB_DT_GPIO_VBUS_5V, gpio_idx, &flags);
    ...
    chip->gpio_VBus5V_active_low = flags == OF_GPIO_ACTIVE_LOW;
    ...
    /* VBUS initial state: off */
    chip->gpio_VBus5V_value = false;
    ret = gpio_direction_output(chip->gpio_VBus5V, chip->gpio_VBus5V_active_low ? 1 : 0);
    if (ret < 0)
    {
    ...
    pr_info("FUSB  %s - VBus 5V initialized as pin '%d' and is set to '%d', %s active\n", __func__, chip->gpio_VBus5V, chip->gpio_VBus5V_value ? 1 : 0,
	    chip->gpio_VBus5V_active_low ? "low" : "high");
...
void fusb_GPIO_Set_VBus5v(FSC_BOOL set)
{
    ...
    value = set ? 1 : 0;
    if (chip->gpio_VBus5V_active_low)
	    value = !value;
    ...
    if (gpio_cansleep(chip->gpio_VBus5V))
    {
         ...
         */
        gpio_set_value_cansleep(chip->gpio_VBus5V, value);
    }
    else
    {
        gpio_set_value(chip->gpio_VBus5V, value);
    }
...
FSC_BOOL fusb_I2C_WriteData(FSC_U8 address, FSC_U8 length, FSC_U8* data)
{
    ...
    for (i = 0; i <= chip->numRetriesI2C; i++)
    {
        ...
        {
            if (ret == -ERANGE) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ERANGE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EINVAL) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EINVAL.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EAGAIN) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EAGAIN.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EALREADY) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EALREADY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EBADE) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EBADE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EBADMSG) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EBADMSG.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EBUSY) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EBUSY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECANCELED) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ECANCELED.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECOMM) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ECOMM.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECONNABORTED) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ECONNABORTED.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECONNREFUSED) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ECONNREFUSED.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECONNRESET) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ECONNRESET.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EDEADLK) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EDEADLK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EDEADLOCK) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EDEADLOCK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EDESTADDRREQ) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EDESTADDRREQ.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EFAULT) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EFAULT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EHOSTDOWN) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EHOSTDOWN.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EHOSTUNREACH) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EHOSTUNREACH.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EILSEQ) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EILSEQ.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EINPROGRESS) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EINPROGRESS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EINTR) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EINTR.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EIO) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EIO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELIBACC) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ELIBACC.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELIBBAD) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ELIBBAD.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELIBMAX) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ELIBMAX.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELOOP) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ELOOP.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EMSGSIZE) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EMSGSIZE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EMULTIHOP) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EMULTIHOP.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOBUFS) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENOBUFS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENODATA) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENODATA.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENODEV) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENODEV.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOLCK) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENOLCK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOMEM) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENOMEM.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOMSG) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENOMSG.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOPROTOOPT) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENOPROTOOPT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOSPC) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENOSPC.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOSYS) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENOSYS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOTBLK) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENOTBLK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOTTY) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENOTTY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOTUNIQ) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENOTUNIQ.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENXIO) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ENXIO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EOVERFLOW) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EOVERFLOW.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPERM) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EPERM.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPFNOSUPPORT) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EPFNOSUPPORT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPIPE) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EPIPE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPROTO) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EPROTO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPROTONOSUPPORT) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EPROTONOSUPPORT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ERANGE) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ERANGE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EREMCHG) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EREMCHG.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EREMOTE) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EREMOTE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EREMOTEIO) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EREMOTEIO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ERESTART) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ERESTART.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ESRCH) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ESRCH.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ETIME) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ETIME.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ETIMEDOUT) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ETIMEDOUT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ETXTBSY) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -ETXTBSY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EUCLEAN) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EUCLEAN.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EUNATCH) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EUNATCH.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EUSERS) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EUSERS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EWOULDBLOCK) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EWOULDBLOCK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EXDEV) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EXDEV.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EXFULL) { pr_err_ratelimited( "FUSB: %s - I2C Error block writing byte data. Address: '0x%02x', Return: -EXFULL.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EOPNOTSUPP) { pr_err_ratelimited( "FUSB: %s - I2C Error writing byte data. Address: '0x%02x', Return: -EOPNOTSUPP.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPROBE_DEFER) { pr_err_ratelimited( "FUSB: %s - I2C Error writing byte data. Address: '0x%02x', Return: -EPROBE_DEFER.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOENT) {
		    pr_err_ratelimited( "FUSB: %s - I2C Error writing byte data. Address: '0x%02x', Return: -ENOENT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C);
		    if (i >= chip->numRetriesI2C)
			    fusb_vote_error_recovery();
	    }
            else { pr_err_ratelimited( "FUSB: %s - Unexpected I2C error block writing byte data. Address: '0x%02x', Return: '%d'.  Attempt #%d / %d...\n", __func__, address, ret, i, chip->numRetriesI2C); }

	    platform_delay_10us(I2C_RETRY_DELAY);
        }
        else                                                                    // Successful i2c writes should always return 0
        {
		fusb_reset_error_recovery();
            break;
        }
    }
...
FSC_BOOL fusb_I2C_ReadData(FSC_U8 address, FSC_U8* data)
{
    ...
    for (i = 0; i <= chip->numRetriesI2C; i++)
    {
        ...
        if (ret < 0)                                                            // Errors report as negative
        {
            if (ret == -ERANGE) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ERANGE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EINVAL) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EINVAL.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EAGAIN) { pr_err_ratelimited( "FUSB: %s - "
                                                                   " reading byte data. Address: '0x%02x', Return: -EAGAIN.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EALREADY) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EALREADY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EBADE) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EBADE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EBADMSG) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EBADMSG.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EBUSY) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EBUSY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECANCELED) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ECANCELED.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECOMM) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ECOMM.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECONNABORTED) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ECONNABORTED.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECONNREFUSED) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ECONNREFUSED.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECONNRESET) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ECONNRESET.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EDEADLK) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EDEADLK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EDEADLOCK) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EDEADLOCK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EDESTADDRREQ) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EDESTADDRREQ.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EFAULT) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EFAULT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EHOSTDOWN) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EHOSTDOWN.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EHOSTUNREACH) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EHOSTUNREACH.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EILSEQ) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EILSEQ.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EINPROGRESS) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EINPROGRESS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EINTR) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EINTR.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EIO) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EIO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELIBACC) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ELIBACC.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELIBBAD) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ELIBBAD.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELIBMAX) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ELIBMAX.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELOOP) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ELOOP.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EMSGSIZE) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EMSGSIZE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EMULTIHOP) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EMULTIHOP.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOBUFS) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOBUFS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENODATA) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENODATA.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENODEV) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENODEV.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOLCK) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOLCK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOMEM) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOMEM.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOMSG) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOMSG.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOPROTOOPT) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOPROTOOPT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOSPC) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOSPC.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOSYS) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOSYS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOTBLK) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOTBLK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOTTY) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOTTY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOTUNIQ) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOTUNIQ.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENXIO) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENXIO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EOVERFLOW) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EOVERFLOW.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPERM) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EPERM.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPFNOSUPPORT) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EPFNOSUPPORT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPIPE) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EPIPE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPROTO) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EPROTO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPROTONOSUPPORT) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EPROTONOSUPPORT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ERANGE) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ERANGE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EREMCHG) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EREMCHG.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EREMOTE) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EREMOTE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EREMOTEIO) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EREMOTEIO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ERESTART) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ERESTART.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ESRCH) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ESRCH.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ETIME) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ETIME.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ETIMEDOUT) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ETIMEDOUT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ETXTBSY) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ETXTBSY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EUCLEAN) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EUCLEAN.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EUNATCH) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EUNATCH.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EUSERS) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EUSERS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EWOULDBLOCK) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EWOULDBLOCK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EXDEV) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EXDEV.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EXFULL) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EXFULL.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EOPNOTSUPP) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EOPNOTSUPP.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPROBE_DEFER) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EPROBE_DEFER.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOENT) {
		    pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOENT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C);
		    if (i >= chip->numRetriesI2C)
			    fusb_vote_error_recovery();
	    }
            else { pr_err_ratelimited( "FUSB: %s - Unexpected I2C error reading byte data. Address: '0x%02x', Return: '%d'.  Attempt #%d / %d...\n", __func__, address, ret, i, chip->numRetriesI2C); }
...
FSC_BOOL fusb_I2C_ReadBlockData(FSC_U8 address, FSC_U8 length, FSC_U8* data)
{
    ...
    for (i = 0; i <= chip->numRetriesI2C; i++)
    {
        ...
        if (ret < 0)                                                                                    // Errors report as negative
        {
            if (ret == -ERANGE) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ERANGE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EINVAL) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EINVAL.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EAGAIN) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EAGAIN.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EALREADY) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EALREADY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EBADE) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EBADE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EBADMSG) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EBADMSG.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EBUSY) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EBUSY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECANCELED) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ECANCELED.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECOMM) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ECOMM.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECONNABORTED) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ECONNABORTED.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECONNREFUSED) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ECONNREFUSED.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ECONNRESET) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ECONNRESET.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EDEADLK) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EDEADLK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EDEADLOCK) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EDEADLOCK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EDESTADDRREQ) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EDESTADDRREQ.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EFAULT) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EFAULT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EHOSTDOWN) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EHOSTDOWN.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EHOSTUNREACH) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EHOSTUNREACH.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EILSEQ) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EILSEQ.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EINPROGRESS) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EINPROGRESS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EINTR) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EINTR.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EIO) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EIO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELIBACC) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ELIBACC.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELIBBAD) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ELIBBAD.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELIBMAX) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ELIBMAX.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ELOOP) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ELOOP.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EMSGSIZE) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EMSGSIZE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EMULTIHOP) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EMULTIHOP.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOBUFS) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENOBUFS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENODATA) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENODATA.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENODEV) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENODEV.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOLCK) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENOLCK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOMEM) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENOMEM.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOMSG) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENOMSG.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOPROTOOPT) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENOPROTOOPT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOSPC) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENOSPC.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOSYS) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENOSYS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOTBLK) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENOTBLK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOTTY) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENOTTY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOTUNIQ) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENOTUNIQ.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENXIO) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ENXIO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EOVERFLOW) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EOVERFLOW.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPERM) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EPERM.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPFNOSUPPORT) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EPFNOSUPPORT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPIPE) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EPIPE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPROTO) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EPROTO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPROTONOSUPPORT) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EPROTONOSUPPORT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ERANGE) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ERANGE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EREMCHG) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EREMCHG.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EREMOTE) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EREMOTE.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EREMOTEIO) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EREMOTEIO.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ERESTART) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ERESTART.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ESRCH) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ESRCH.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ETIME) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ETIME.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ETIMEDOUT) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ETIMEDOUT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ETXTBSY) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -ETXTBSY.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EUCLEAN) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EUCLEAN.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EUNATCH) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EUNATCH.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EUSERS) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EUSERS.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EWOULDBLOCK) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EWOULDBLOCK.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EXDEV) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EXDEV.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EXFULL) { pr_err_ratelimited( "FUSB: %s - I2C Error block reading byte data. Address: '0x%02x', Return: -EXFULL.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EOPNOTSUPP) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EOPNOTSUPP.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -EPROBE_DEFER) { pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -EPROBE_DEFER.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C); }
            else if (ret == -ENOENT) {
		    pr_err_ratelimited( "FUSB: %s - I2C Error reading byte data. Address: '0x%02x', Return: -ENOENT.  Attempt #%d / %d...\n", __func__, address, i, chip->numRetriesI2C);
            ...
            else { pr_err_ratelimited( "FUSB: %s - Unexpected I2C error block reading byte data. Address: '0x%02x', Return: '%d'.  Attempt #%d / %d...\n", __func__, address, ret, i, chip->numRetriesI2C); }

	    platform_delay_10us(I2C_RETRY_DELAY);
        }
        else if (ret != length) // We didn't read everything we wanted
        {
            pr_err_ratelimited( "FUSB: %s - Error: Block read request of %u bytes truncated to %u bytes.\n", __func__, length, I2C_SMBUS_BLOCK_MAX);
        }
        else
        {
		fusb_reset_error_recovery();
...
static ssize_t _fusb_Sysfs_PDStateLog_show(struct device* dev, struct device_attribute* attr, char* buf)
{
    ...
    for (i = 4; (i + 4 < FSC_HOSTCOMM_BUFFER_SIZE) && (numChars < PAGE_SIZE) && (numLogs > 0); i += 5, numLogs--) // Must be able to peek 4 bytes ahead, and don't overflow the output buffer (PAGE_SIZE)
    {
        ...
        switch (output[i])
        {
            case peDisabled:		    // Policy engine is disabled
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDisabled\t\tPolicy engine is disabled\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peErrorRecovery:		// Error recovery state
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeErrorRecovery\t\tError recovery state\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceHardReset:		// Received a hard reset
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceHardReset\t\tReceived a hard reset\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceSendHardReset:		// Source send a hard reset
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceSendHardReset\t\tSource send a hard reset\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceSoftReset:		// Received a soft reset
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceSoftReset\t\tReceived a soft reset\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceSendSoftReset:		// Send a soft reset
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceSendSoftReset\t\tSend a soft reset\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceStartup:		// Initial state
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceStartup\t\tInitial state\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceSendCaps:		// Send the source capabilities
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceSendCaps\t\tSend the source capabilities\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceDiscovery:		// Waiting to detect a USB PD sink
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceDiscovery\t\tWaiting to detect a USB PD sink\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceDisabled:		// Disabled state
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceDisabled\t\tDisabled state\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceTransitionDefault:		// Transition to default 5V state
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceTransitionDefault\t\tTransition to default 5V state\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceNegotiateCap:		// Negotiate capability and PD contract
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceNegotiateCap\t\tNegotiate capability and PD contract\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceCapabilityResponse:		// Respond to a request message with a reject/wait
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceCapabilityResponse\t\tRespond to a request message with a reject/wait\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceTransitionSupply:		// Transition the power supply to the new setting (accept request)
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceTransitionSupply\t\tTransition the power supply to the new setting (accept request)\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceReady:		// Contract is in place and output voltage is stable
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceReady\t\tContract is in place and output voltage is stable\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceGiveSourceCaps:		// State to resend source capabilities
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceGiveSourceCaps\t\tState to resend source capabilities\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceGetSinkCaps:		// State to request the sink capabilities
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceGetSinkCaps\t\tState to request the sink capabilities\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceSendPing:		// State to send a ping message
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceSendPing\t\tState to send a ping message\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceGotoMin:		// State to send the gotoMin and ready the power supply
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceGotoMin\t\tState to send the gotoMin and ready the power supply\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceGiveSinkCaps:		// State to send the sink capabilities if dual-role
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceGiveSinkCaps\t\tState to send the sink capabilities if dual-role\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceGetSourceCaps:		// State to request the source caps from the UFP
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceGetSourceCaps\t\tState to request the source caps from the UFP\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceSendDRSwap:		// State to send a DR_Swap message
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceSendDRSwap\t\tState to send a DR_Swap message\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceEvaluateDRSwap:		// Evaluate whether we are going to accept or reject the swap
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceEvaluateDRSwap\t\tEvaluate whether we are going to accept or reject the swap\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkHardReset:		// Received a hard reset
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkHardReset\t\tReceived a hard reset\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkSendHardReset:		// Sink send hard reset
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkSendHardReset\t\tSink send hard reset\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkSoftReset:		// Sink soft reset
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkSoftReset\t\tSink soft reset\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkSendSoftReset:		// Sink send soft reset
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkSendSoftReset\t\tSink send soft reset\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkTransitionDefault:		// Transition to the default state
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkTransitionDefault\t\tTransition to the default state\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkStartup:		// Initial sink state
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkStartup\t\tInitial sink state\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkDiscovery:		// Sink discovery state
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkDiscovery\t\tSink discovery state\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkWaitCaps:		// Sink wait for capabilities state
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkWaitCaps\t\tSink wait for capabilities state\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkEvaluateCaps:		// Sink state to evaluate the received source capabilities
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkEvaluateCaps\t\tSink state to evaluate the received source capabilities\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkSelectCapability:		// Sink state for selecting a capability
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkSelectCapability\t\tSink state for selecting a capability\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkTransitionSink:		// Sink state for transitioning the current power
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkTransitionSink\t\tSink state for transitioning the current power\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkReady:		// Sink ready state
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkReady\t\tSink ready state\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkGiveSinkCap:		// Sink send capabilities state
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkGiveSinkCap\t\tSink send capabilities state\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkGetSourceCap:		// Sink get source capabilities state
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkGetSourceCap\t\tSink get source capabilities state\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkGetSinkCap:		// Sink state to get the sink capabilities of the connected source
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkGetSinkCap\t\tSink state to get the sink capabilities of the connected source\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkGiveSourceCap:		// Sink state to send the source capabilities if dual-role
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkGiveSourceCap\t\tSink state to send the source capabilities if dual-role\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkSendDRSwap:		// State to send a DR_Swap message
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkSendDRSwap\t\tState to send a DR_Swap message\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkEvaluateDRSwap:		// Evaluate whether we are going to accept or reject the swap
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkEvaluateDRSwap\t\tEvaluate whether we are going to accept or reject the swap\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceSendVCONNSwap:		// Initiate a VCONN swap sequence
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceSendVCONNSwap\t\tInitiate a VCONN swap sequence\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkEvaluateVCONNSwap:		// Evaluate whether we are going to accept or reject the swap
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkEvaluateVCONNSwap\t\tEvaluate whether we are going to accept or reject the swap\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceSendPRSwap:		// Initiate a PR_Swap sequence
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceSendPRSwap\t\tInitiate a PR_Swap sequence\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceEvaluatePRSwap:		// Evaluate whether we are going to accept or reject the swap
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceEvaluatePRSwap\t\tEvaluate whether we are going to accept or reject the swap\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkSendPRSwap:		// Initiate a PR_Swap sequence
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkSendPRSwap\t\tInitiate a PR_Swap sequence\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSinkEvaluatePRSwap:		// Evaluate whether we are going to accept or reject the swap
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSinkEvaluatePRSwap\t\tEvaluate whether we are going to accept or reject the swap\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peGiveVdm:		// Send VDM data
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeGiveVdm\t\tSend VDM data\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmGetIdentity:		// Requesting Identity information from DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmGetIdentity\t\tRequesting Identity information from DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmSendIdentity:		// Sending Discover Identity ACK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmSendIdentity\t\tSending Discover Identity ACK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmGetSvids:		// Requesting SVID info from DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmGetSvids\t\tRequesting SVID info from DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmSendSvids:		// Sending Discover SVIDs ACK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmSendSvids\t\tSending Discover SVIDs ACK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmGetModes:		// Requesting Mode info from DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmGetModes\t\tRequesting Mode info from DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmSendModes:		// Sending Discover Modes ACK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmSendModes\t\tSending Discover Modes ACK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmEvaluateModeEntry:		// Requesting DPM to evaluate request to enter a mode, and enter if OK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmEvaluateModeEntry\t\tRequesting DPM to evaluate request to enter a mode, and enter if OK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmModeEntryNak:		// Sending Enter Mode NAK response
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmModeEntryNak\t\tSending Enter Mode NAK response\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmModeEntryAck:		// Sending Enter Mode ACK response
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmModeEntryAck\t\tSending Enter Mode ACK response\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmModeExit:		// Requesting DPM to evalute request to exit mode
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmModeExit\t\tRequesting DPM to evalute request to exit mode\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmModeExitNak:		// Sending Exit Mode NAK reponse
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmModeExitNak\t\tSending Exit Mode NAK reponse\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmModeExitAck:		// Sending Exit Mode ACK Response
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmModeExitAck\t\tSending Exit Mode ACK Response\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peUfpVdmAttentionRequest:		// Sending Attention Command
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeUfpVdmAttentionRequest\t\tSending Attention Command\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpUfpVdmIdentityRequest:		// Sending Identity Request
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpUfpVdmIdentityRequest\t\tSending Identity Request\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpUfpVdmIdentityAcked:		// Inform DPM of Identity
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpUfpVdmIdentityAcked\t\tInform DPM of Identity\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpUfpVdmIdentityNaked:		// Inform DPM of result
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpUfpVdmIdentityNaked\t\tInform DPM of result\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpCblVdmIdentityRequest:		// Sending Identity Request
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpCblVdmIdentityRequest\t\tSending Identity Request\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpCblVdmIdentityAcked:		// Inform DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpCblVdmIdentityAcked\t\tInform DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpCblVdmIdentityNaked:		// Inform DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpCblVdmIdentityNaked\t\tInform DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmSvidsRequest:		// Sending Discover SVIDs request
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmSvidsRequest\t\tSending Discover SVIDs request\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmSvidsAcked:		// Inform DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmSvidsAcked\t\tInform DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmSvidsNaked:		// Inform DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmSvidsNaked\t\tInform DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmModesRequest:		// Sending Discover Modes request
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmModesRequest\t\tSending Discover Modes request\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmModesAcked:		// Inform DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmModesAcked\t\tInform DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmModesNaked:		// Inform DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmModesNaked\t\tInform DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmModeEntryRequest:		// Sending Mode Entry request
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmModeEntryRequest\t\tSending Mode Entry request\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmModeEntryAcked:		// Inform DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmModeEntryAcked\t\tInform DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmModeEntryNaked:		// Inform DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmModeEntryNaked\t\tInform DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmModeExitRequest:		// Sending Exit Mode request
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmModeExitRequest\t\tSending Exit Mode request\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmExitModeAcked:		// Inform DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmExitModeAcked\t\tInform DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSrcVdmIdentityRequest:		// sending Discover Identity request
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSrcVdmIdentityRequest\t\tsending Discover Identity request\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSrcVdmIdentityAcked:		// inform DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSrcVdmIdentityAcked\t\tinform DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSrcVdmIdentityNaked:		// inform DPM
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSrcVdmIdentityNaked\t\tinform DPM\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDfpVdmAttentionRequest:		// Attention Request received
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDfpVdmAttentionRequest\t\tAttention Request received\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblReady:		// Cable power up state?
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblReady\t\tCable power up state?\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblGetIdentity:		// Discover Identity request received
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblGetIdentity\t\tDiscover Identity request received\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblGetIdentityNak:		// Respond with NAK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblGetIdentityNak\t\tRespond with NAK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblSendIdentity:		// Respond with Ack
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblSendIdentity\t\tRespond with Ack\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblGetSvids:		// Discover SVIDs request received
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblGetSvids\t\tDiscover SVIDs request received\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblGetSvidsNak:		// Respond with NAK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblGetSvidsNak\t\tRespond with NAK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblSendSvids:		// Respond with ACK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblSendSvids\t\tRespond with ACK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblGetModes:		// Discover Modes request received
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblGetModes\t\tDiscover Modes request received\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblGetModesNak:		// Respond with NAK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblGetModesNak\t\tRespond with NAK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblSendModes:		// Respond with ACK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblSendModes\t\tRespond with ACK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblEvaluateModeEntry:		// Enter Mode request received
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblEvaluateModeEntry\t\tEnter Mode request received\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblModeEntryAck:		// Respond with NAK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblModeEntryAck\t\tRespond with NAK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblModeEntryNak:		// Respond with ACK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblModeEntryNak\t\tRespond with ACK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblModeExit:		// Exit Mode request received
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblModeExit\t\tExit Mode request received\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblModeExitAck:		// Respond with NAK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblModeExitAck\t\tRespond with NAK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peCblModeExitNak:		// Respond with ACK
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeCblModeExitNak\t\tRespond with ACK\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peDpRequestStatus:		// Requesting PP Status
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeDpRequestStatus\t\tRequesting PP Status\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case PE_BIST_Receive_Mode:		// Bist Receive Mode
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tPE_BIST_Receive_Mode\t\tBist Receive Mode\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case PE_BIST_Frame_Received:		// Test Frame received by Protocol layer
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tPE_BIST_Frame_Received\t\tTest Frame received by Protocol layer\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case PE_BIST_Carrier_Mode_2:		// BIST Carrier Mode 2
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tPE_BIST_Carrier_Mode_2\t\tBIST Carrier Mode 2\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case peSourceWaitNewCapabilities:		// Wait for new Source Capabilities from Policy Manager
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tpeSourceWaitNewCapabilities\t\tWait for new Source Capabilities from Policy Manager\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
           ...
           //   numChars += sprintf(tempBuf, "%02u|0.%04u\tdbgGetRxPacket\t\t\tNumber of I2C bytes read | Time elapsed\n", TimeStampSeconds, TimeStampMS10ths);
            ...
            default:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tUKNOWN STATE: 0x%02x\n", TimeStampSeconds, TimeStampMS10ths, output[i]);
                strcat(buf, tempBuf);
                break;
            }
        }
...
static ssize_t _fusb_Sysfs_TypeCStateLog_show(struct device* dev, struct device_attribute* attr, char* buf)
{
    ...
    for (i = 4; (i + 4 < FSC_HOSTCOMM_BUFFER_SIZE) && (numChars < PAGE_SIZE) && (numLogs > 0); i += 5, numLogs--) // Must be able to peek 4 bytes ahead, and don't overflow the output buffer (PAGE_SIZE), only print logs we have
    {
        ...
        switch (output[i])
        {
            case Disabled:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tDisabled\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case ErrorRecovery:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tErrorRecovery\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case Unattached:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tUnattached\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case AttachWaitSink:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tAttachWaitSink\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case AttachedSink:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tAttachedSink\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case AttachWaitSource:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tAttachWaitSource\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case AttachedSource:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tAttachedSource\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case TrySource:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tTrySource\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case TryWaitSink:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tTryWaitSink\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case TrySink:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tTrySink\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case TryWaitSource:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tTryWaitSource\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case AudioAccessory:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tAudioAccessory\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
	    ...
	    case DebugAccessorySource:
	    {
		    numChars += sprintf(tempBuf, "[%u.%03u]\tDebugAccessorySource\n", TimeStampSeconds, TimeStampMS10ths);
		    strcat(buf, tempBuf);
		    break;
	    }
	    ...
	    case DebugAccessorySink:
	    {
		    numChars += sprintf(tempBuf, "[%u.%03u]\tDebugAccessorySink\n", TimeStampSeconds, TimeStampMS10ths);
		    strcat(buf, tempBuf);
		    break;
	    }
            ...
            case AttachWaitAccessory:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tAttachWaitAccessory\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case PoweredAccessory:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tPoweredAccessory\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case UnsupportedAccessory:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tUnsupportedAccessory\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case DelayUnattached:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tDelayUnattached\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            ...
            case UnattachedSource:
            {
                numChars += sprintf(tempBuf, "[%u.%03u]\tUnattachedSource\n", TimeStampSeconds, TimeStampMS10ths);
                strcat(buf, tempBuf);
                break;
            }
            default:
...
static ssize_t _fusb_Sysfs_Reinitialize_fusb302(struct device* dev, struct device_attribute* attr, char* buf)
{
    ...
    // comment out since timer has been start in core_initialize/InitializeTypeC/SetStateDelayUnattached
    // fusb_StartTimers();
...
FSC_S32 fusb_EnableInterrupts(void)
{
    ...
    wake_lock_init(&chip->cclock, WAKE_LOCK_SUSPEND, "cc_irq");
...
static irqreturn_t _fusb_isr_intn(FSC_S32 irq, void *dev_id)
{
    ...
    do
    {
        ...
        if (g_Unattach_Fail) {
            if (g_Illegal_Cable) {
                pr_info("%s [Illegal Cable/Dongle Recovery Method] Illegal Cable/Dongle failure - disabling state machine \n", __func__);
                g_Illegal_Cable = FALSE;
                core_enable_typec(FALSE);
                platform_delay_10us(ILLEGAL_RETRY_DELAY);
       	    } else {
                pr_info("%s [Illegal Cable/Dongle Recovery Method] Illegal Cable/Dongle failure - re-enabling state machine \n", __func__);
                g_Unattach_Fail = FALSE;
                fusb_InitializeCore();
       	    }
        }
        core_state_machine();             // Run the state machine
    } while(!g_I2C_Fail && platform_get_device_irq_state());

    wake_unlock(&chip->cclock);
...
void _fusb_intWorker(struct work_struct* work)
{
    struct fusb30x_chip* chip = container_of(work, struct fusb30x_chip, worker);
    if (!chip)
    {
    ...
    do {
	    core_state_machine(); // Run the state machine
    } while(platform_get_device_irq_state() && !g_I2C_Fail);
...
#ifdef FSC_INTERRUPT_TRIGGERED
    INIT_WORK(&chip->worker, _fusb_intWorker);
#else
...
enum typec_attached_state fusb_get_connecting_state(struct connecting_state_descriptor *desc)
{
	...
	do {
		cstate = core_get_connecting_state(desc);
		switch (cstate) {
		case AttachedSink:
...
void fusb_vbus_off(FSC_BOOL off)
{
	...
	if (off) {
		...
		do {
			if (VbusVSafe0V())
				break;
			platform_delay_10us(100*10);
		} while (time_before(jiffies, timeout));
	} else {
		fusb_GPIO_Set_VBus5v(true);
		//fusb_GPIO_Set_VBusOther(false);
		timeout = jiffies + HZ/2;
		do {
			if (isVSafe5V())
...
void fusb_notify_state_chaged(ConnectionState previous_state, ConnectionState current_state)
{
	...
	if (previous_state == AttachedSink && current_state == Unattached)
		fusb_notify_pd_sink_reset();

	if (chip->reverse_state == REVERSE_ATTEMPT) {
		switch (current_state) {
		case AttachedSource:
			pr_err("FUSB  %s - reverse completed, set REVERSE_DEBOUNCE\n", __func__);
...
int fusb_notify_pd_sink_ready(FSC_U32 pdo_object)
{
	...
	err = typec_pd_on_sink_ready(pdo_object);
#ifdef FSC_DEBUG
	pr_info("FUSB  %s - notify PDO: 0x%08x %s\n", __func__,
		pdo_object, err ? "fail" : "succeed");
#endif
	return err;
}
...
#if 0
	/* force report zero. for debugging only */
	return 0;
#else
	return atomic_read(&chip->latest_batt_capacity);
#endif
}

void fusb_handle_external_event(external_event evt)
{
	if (evt >= EXTEVT_INVALID)
		return;
	typec_handle_external_event(evt);
}
/**
 * Call from platform.c
 */
void fusb_pd_queue_delayed_work(int ms)
{
	...
	cancel_delayed_work(&chip->pd_batt_cap_change_work);
	queue_delayed_work(system_nrt_freezable_wq, &chip->pd_batt_cap_change_work,
		msecs_to_jiffies(ms));
}

#define NO_BUS_BOOST false
#define NO_PM_QOS true
static void fusb_bus_clks_enable(struct fusb30x_chip* chip)
{
	int i;
	int ret;

	/* Because of boost buses drains more power,
	 * ~1.3W measured in Z500KL, we disable it until
	 * we have other solutions in thermal. */
	if (NO_BUS_BOOST || chip->bus_clks_enabled)
		return;

	for (i = 0; i < FUSB_NUM_BUS_CLOCKS; i++) {
		ret = clk_prepare_enable(chip->bus_clks[i]);
		if (ret) {
	...
	chip->bus_clks_enabled = true;
	return;
err_clk_en:
	for (--i; i >= 0; --i)
		clk_disable_unprepare(chip->bus_clks[i]);
}

static void fusb_bus_clks_disable(struct fusb30x_chip* chip)
{
	int i;

	if (!chip->bus_clks_enabled)
		return;
...
static void fusb_pm_qos_update_latency(struct fusb30x_chip* chip, s32 latency)
{
	 ...
	 * ~1.3W measured in Z500KL, we disable it until
	 * we have other solutions in thermal. */
	if (NO_PM_QOS || latency == last_vote || !latency)
		return;

	pr_debug("%s: latency updated to: %d\n", __func__, latency);

	pm_qos_update_request(&chip->pm_qos_req_dma, latency);

	last_vote = latency;
}

/**
 * Call from platform.c to reduce latency. Do things as follows.
 * 1. maximum BUS clk rate.
 * 2. vote pm_qos based on kernel pm_qos framework
 */
void fusb_pm_qos_enable(FSC_BOOL enable)
{
	...
	if (enable) {
		fusb_bus_clks_enable(chip);
		fusb_pm_qos_update_latency(chip, chip->pm_qos_latency);
	} else {
		fusb_pm_qos_update_latency(chip, PM_QOS_DEFAULT_VALUE);
		fusb_bus_clks_disable(chip);
	}
}

bool fusb_pd_wait_for_source_caps(void)
{
	return typec_pd_wait_for_source_caps();
}

bool fusb_pd_is_pr_swapable(void)
{
	return typec_pd_is_pr_swapable();
}
