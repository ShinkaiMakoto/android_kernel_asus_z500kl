...
int mchgStatus=-1;
enum power_supply_type initTouch= POWER_SUPPLY_TYPE_UNKNOWN;
...
static void notifyTouch(enum power_supply_type type){
    ...
    if(mchgStatus!=tempStatus){
		printk("%s: notify %d",__func__,mchgStatus);
		ft5726_cable_status_handler(mchgStatus);
    }
#endif
}
...
static DECLARE_COMPLETION(sm_init);
bool wait_usb_phy_ready(void)
{
	int ret;
	struct msm_otg *motg = the_msm_otg;
	bool ready = true;

	if (!motg || motg->phy.state == OTG_STATE_UNDEFINED)
		ready = false;

	if (!ready) {
		ret = wait_for_completion_timeout(&sm_init, msecs_to_jiffies(20000));
		if (!ret) {
			pr_err("%s: wait state machine ready timeout\n", __func__);
			return false;
		}
	}
	return true;
}
EXPORT_SYMBOL(wait_usb_phy_ready);

static void
msm_otg_dbg_log_event(struct usb_phy *phy, char *event, int d1, int d2);

int usb_peripheral_mode(bool on)
{
	struct msm_otg *motg = the_msm_otg;
	...
	if (!motg)
		return -EPROBE_DEFER;
	if (on) {
		queue_delayed_work(motg->otg_wq, &motg->pmic_work, HZ);
	} else {
		cancel_delayed_work_sync(&motg->pmic_work);
		if (test_and_clear_bit(B_SESS_VLD, &motg->inputs)) {
			pr_debug("B_SESS_VLD CLAER\n");
			msm_otg_dbg_log_event(&motg->phy, "B_SESS_VLD CLEAR (TypeC)",
...
EXPORT_SYMBOL(usb_peripheral_mode);
...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	struct msm_otg_platform_data *pdata = motg->pdata;
	int cnt;
	bool host_bus_suspend, device_bus_suspend, dcp, prop_charger, pd_charger;
...
lpm_start:
	...
	pd_charger = motg->chg_type == USB_PD_CHARGER;
	...
	if ((test_bit(B_SESS_VLD, &motg->inputs) &&
		...
		!prop_charger && !floated_charger && !pd_charger) ||
...
static void msm_otg_chg_check_timer_func(unsigned long data)
{
	...
	if (typec_pd_online(&pd_usb_comm_capable) && otg->gadget->speed == USB_SPEED_UNKNOWN) {
		if (pd_usb_comm_capable)
			return;	/* Don't report B_FALSE_SDP if the peer is able to communicate via USB. */

		dev_dbg(otg->phy->dev, "PD is detected as SDP\n");
		msm_otg_dbg_log_event(&motg->phy, "PD IS DETECTED AS SDP",
				      otg->phy->state, 0);
		set_bit(B_FALSE_SDP, &motg->inputs);
		queue_work(motg->otg_wq, &motg->sm_work);
	}
...
static const char *chg_to_string(enum usb_chg_type chg_type)
{
	...
	case USB_PD_CHARGER: 		return "USB_PD_CHARGER";
	...
	case OTG_STATE_B_PERIPHERAL:
		if (test_bit(B_SESS_VLD, &motg->inputs) &&
			...
			msm_otg_dbg_log_event(&motg->phy, "B_FALSE_SDP",
					motg->inputs, otg->phy->state);
			msm_otg_start_peripheral(otg, 0);
			if (typec_pd_online(NULL))
				motg->chg_type = USB_PD_CHARGER;
			else
				motg->chg_type = USB_DCP_CHARGER;
			clear_bit(B_FALSE_SDP, &motg->inputs);
			otg->phy->state = OTG_STATE_B_IDLE;
...
static void msm_wait_pmic_sbc_done_work(struct work_struct *w)
{
	unsigned long timeout;
	struct msm_otg *motg =
		container_of(w, struct msm_otg, pmic_work.work);

	timeout = jiffies + (3*HZ);
	do {
		pr_debug("%s: motg->chg_state = %d\n", __func__, motg->chg_state);
	...
	set_bit(B_SESS_VLD, &motg->inputs);
	pr_debug("B_SESS_VLD\n");
	msm_otg_dbg_log_event(&motg->phy, "B_SESS_VLD (TypeC)",
			      motg->inputs, motg->phy.state);
	queue_work(motg->otg_wq, &motg->sm_work);
}
...
#ifdef CONFIG_FUSB_30X
		/* We have PD capability, skip calling power_supply_changed() here if
		 * this connection is 'sink attached'. This avoid charger works in advance of
		 * PD/PS ready, improve timing in PD protocol handshake. */
		if (motg->pd_psy == NULL)
			motg->pd_psy = power_supply_get_by_name("power_delivery");
		if (motg->pd_psy) {
			motg->pd_psy->get_property(motg->pd_psy, POWER_SUPPLY_PROP_ONLINE, &value);
			if (value.intval) {
				/* PD ONLINE */
				motg->pd_psy->get_property(motg->pd_psy, POWER_SUPPLY_PROP_TYPEC_MODE, &value);
				if (value.intval == POWER_SUPPLY_TYPEC_MODE_SINK)
					need_to_update_ps = false;
			}
		}
#endif
		break;
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&motg->pmic_work, msm_wait_pmic_sbc_done_work);
