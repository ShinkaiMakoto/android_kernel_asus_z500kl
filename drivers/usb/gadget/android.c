					...
					struct usb_composite_dev *cdev)
{
	...
	//ZT500KL close uicc disk for mass storage function
...
static ssize_t speed_show(struct device *pdev, struct device_attribute *attr,
			   char *buf)
{
	struct android_dev *dev = dev_get_drvdata(pdev);
	struct usb_composite_dev *cdev = dev->cdev;
...
static DEVICE_ATTR(speed, S_IRUGO, speed_show, NULL);
