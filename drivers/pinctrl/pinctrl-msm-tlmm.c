...
#ifdef CONFIG_CUS_WAKEUP_SOURCE
        struct msm_pintype_info *pinfo = ic_to_pintype(ic);
        struct gpio_chip *gc = pintype_get_gc(pinfo);
        unsigned int virq = 0;
        struct irq_desc *desc_name;
        const char *name = "null";

        for_each_set_bit(i, ic->wake_irqs, num_irqs){
                if (msm_tlmm_get_intr_status(ic, i)) {
                        virq = msm_tlmm_gp_to_irq(gc, i);
                        desc_name = irq_to_desc(virq);
                        if (desc_name == NULL) name = "stay";
                        else if (desc_name->action && desc_name->action->name)
                                name = desc_name->action->name;
                        pr_err("virq %d triggered(%s)\n", virq,name);
