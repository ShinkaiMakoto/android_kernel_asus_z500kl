...
static inline int tcpci_sink_vbus_prerequisite(
	struct tcpc_device *tcpc, uint32_t pdo)
{
	struct tcp_notify tcp_noti;

	tcp_noti.pdo_tobe_request.pdo = pdo;
	TCPC_DBG("notify prerequisite operation of sink_vbus. pdo = %x\r\n", pdo);
	return srcu_notifier_call_chain(&tcpc->evt_nh,
				TCP_NOTIFY_SINK_VBUS_PREREQUISITE, &tcp_noti);
}
