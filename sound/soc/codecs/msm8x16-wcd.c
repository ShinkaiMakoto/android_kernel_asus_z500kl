...
struct delayed_work hp_delay_l, hp_delay_r;
static struct snd_soc_codec *codec1;
static struct snd_soc_dapm_widget *w1;
...
static int rspk_amp_gpio = -1;
static int rspk_5v_gpio = -1;
...
static int msm8x16_wcd_right_spk_get(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
       pr_debug("%s : rspk_amp_gpio %d value %d\n", __func__, rspk_amp_gpio, gpio_get_value(rspk_amp_gpio));

       if (gpio_get_value(rspk_amp_gpio) == 1)
               ucontrol->value.integer.value[0] = 1;
       else
               ucontrol->value.integer.value[0] = 0;
...
static int msm8x16_wcd_right_spk_put(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
       pr_debug("%s : rspk_amp_gpio %d value %d\n", __func__, rspk_amp_gpio, gpio_get_value(rspk_amp_gpio));
       ...
       switch (ucontrol->value.integer.value[0]) {
       case 0:
               gpio_set_value_cansleep(rspk_amp_gpio, 0);
               break;
       case 1:
               gpio_set_value_cansleep(rspk_amp_gpio, 1);
               break;
       default:
               return -EINVAL;
       }
...
static int msm8x16_wcd_right_spk_5v_get(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
       pr_debug("%s : rspk_5v_gpio %d value %d\n", __func__, rspk_amp_gpio, gpio_get_value(rspk_amp_gpio));

       if (gpio_get_value(rspk_5v_gpio) == 1)
               ucontrol->value.integer.value[0] = 1;
       else
               ucontrol->value.integer.value[0] = 0;
...
static int msm8x16_wcd_right_spk_5v_put(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
       pr_debug("%s : rspk_5v_gpio %d value %d\n", __func__, rspk_amp_gpio, gpio_get_value(rspk_amp_gpio));
       ...
       switch (ucontrol->value.integer.value[0]) {
       case 0:
               gpio_set_value_cansleep(rspk_5v_gpio, 0);
               break;
       case 1:
               gpio_set_value_cansleep(rspk_5v_gpio, 1);
               break;
       default:
               return -EINVAL;
       }
...
// joe_cheng
static const char * const msm8x16_wcd_right_spk_ctrl_text[] = {
               "DISABLE", "ENABLE"};
static const struct soc_enum msm8x16_wcd_right_spk_ctl_enum[] = {
               SOC_ENUM_SINGLE_EXT(2, msm8x16_wcd_right_spk_ctrl_text),
};

static const char * const msm8x16_wcd_right_spk_5v_ctrl_text[] = {
               "DISABLE", "ENABLE"};
static const struct soc_enum msm8x16_wcd_right_spk_5v_ctl_enum[] = {
               SOC_ENUM_SINGLE_EXT(2, msm8x16_wcd_right_spk_5v_ctrl_text),
};
...
static const struct snd_kcontrol_new msm8x16_wcd_snd_controls[] = {
	...
	SOC_ENUM_EXT("LOOPBACK Mode", msm8x16_wcd_loopback_mode_ctl_enum[0],
        ...
        // joe_cheng
        SOC_ENUM_EXT("Right Speaker AMP", msm8x16_wcd_right_spk_ctl_enum[0],
                msm8x16_wcd_right_spk_get, msm8x16_wcd_right_spk_put),

	SOC_ENUM_EXT("Right Speaker 5v", msm8x16_wcd_right_spk_5v_ctl_enum[0],
                msm8x16_wcd_right_spk_5v_get, msm8x16_wcd_right_spk_5v_put),
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (PROJ_ID == 0x4){ //For ZT581KL
			//Set CDC_A_BOOST_OUTPUT_VOLTAGE to 4V
...
static void msm8x16_wcd_close_spk_path(struct snd_soc_dapm_widget *w)
{
	struct snd_soc_codec *codec = w->codec;
        struct msm8x16_wcd_priv *msm8x16_wcd = snd_soc_codec_get_drvdata(codec);
	// Same as msm8x16_wcd_codec_enable_spk_pa PRE_PMD and POST_PMD
	snd_soc_update_bits(codec,
		MSM8X16_WCD_A_CDC_RX3_B6_CTL, 0x01, 0x01);
	usleep_range(CODEC_DELAY_1_MS, CODEC_DELAY_1_1_MS);
	snd_soc_update_bits(codec,
						 ...
						 int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if ((w->shift == 0) &&
			...
			//Eddy_Liao : Close speaker fot ZT581KL
			pr_debug("Close speaker fot ZT581KL\n");
			msm8x16_wcd_close_spk_path(w);
			//Close speaker fot ZT581KL END
			snd_soc_update_bits(codec,
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (w->shift == 5) {
			...
			schedule_delayed_work(&hp_delay_l,msecs_to_jiffies(200)); //Delay open headset_l
			//snd_soc_update_bits(codec,
				//MSM8X16_WCD_A_CDC_RX1_B6_CTL, 0x01, 0x00);
		} else if (w->shift == 4) {
			snd_soc_update_bits(codec,
				MSM8X16_WCD_A_ANALOG_RX_HPH_R_TEST, 0x04, 0x04);
			schedule_delayed_work(&hp_delay_r,msecs_to_jiffies(200)); //Delay open headset_r
			//snd_soc_update_bits(codec,
				//MSM8X16_WCD_A_CDC_RX2_B6_CTL, 0x01, 0x00);
		}
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_update_bits(codec,
			MSM8X16_WCD_A_CDC_RX1_B6_CTL, 0x01, 0x00);
		if (PROJ_ID == 0x1 || PROJ_ID == 0x3){ //For ZT500KL ans Z500KL
			// joe_cheng : Workaround for earpiece pop sound
			gpio_set_value_cansleep(rspk_amp_gpio, 1);
		}
		break;
	case SND_SOC_DAPM_PRE_PMD:
...
static void do_hp_delay_l(struct work_struct *work)
{
	//pr_err("delay hpl");
	snd_soc_update_bits(codec1,
		MSM8X16_WCD_A_ANALOG_RX_HPH_L_TEST, 0x04, 0x04);
	snd_soc_update_bits(codec1,
		MSM8X16_WCD_A_CDC_RX1_B6_CTL, 0x01, 0x00);
}

static void do_hp_delay_r(struct work_struct *work)
{
        //pr_err("delay hpr");
	snd_soc_update_bits(codec1,
		MSM8X16_WCD_A_ANALOG_RX_HPH_R_TEST, 0x04, 0x04);
...
static int msm8x16_wcd_spmi_probe(struct spmi_device *spmi)
{
	...
	int adsp_state;
        const char *rspk_amp = "qcom,right-spk-amp";
	const char *rspk_5v = "qcom,right-spk-5v";
	...
	if (pdata == NULL) {
        ...
        rspk_amp_gpio = of_get_named_gpio(spmi->dev.of_node, rspk_amp, 0);
        if (rspk_amp_gpio < 0) {
                pr_err("%s: missing %s in dt node\n", __func__, rspk_amp);
        } else {
                ret = gpio_request(rspk_amp_gpio, "right spk amp");
                if (ret < 0)
                        pr_err("%s : Request right speaker amp gpio %d failed\n", __func__, rspk_amp_gpio);
                else {
                        gpio_direction_output(rspk_amp_gpio, 0);
                        gpio_set_value_cansleep(rspk_amp_gpio, 0);
                }
        }

	if(PROJ_ID == 0) { //For ZT582KL left speaker
		rspk_5v_gpio = of_get_named_gpio(spmi->dev.of_node, rspk_5v, 0);
        	if (rspk_5v_gpio < 0) {
                	pr_err("%s: missing %s in dt node\n", __func__, rspk_amp);
        	} else {
                	ret = gpio_request(rspk_5v_gpio, "right spk 5v");
                	if (ret < 0)
                        	pr_err("%s : Request right speaker 5v gpio %d failed\n", __func__, rspk_amp_gpio);
                	else {
                        	gpio_direction_output(rspk_5v_gpio, 0);
                        	gpio_set_value_cansleep(rspk_5v_gpio, 0);
                	}
        	}
	}
	...
	INIT_DELAYED_WORK(&hp_delay_l, do_hp_delay_l);
	INIT_DELAYED_WORK(&hp_delay_r, do_hp_delay_r);
